...
| Provider descriptors | `netEaseSourceProviderDescriptor`, `netEasePlatformLibraryProviderDescriptor` in `src/providers/netease/index.ts` |
...
slots: `source` and `platform_library`. The platform-library provider also
implements optional paged reads through `readPage(...)`, which Library Import
uses for restart-safe continuation batches.
...
Saved tracks use the user's liked-music playlist detail as the provider item
fact source. The adapter discovers the liked playlist, reads
`/playlist/detail`, uses `playlist.trackIds` order as newest-first source order,
and uses each `trackIds[].at` value as provider add time when present. The
provider then calls `/song/detail` for required song facts. `/likelist` is not
the current Source Library track import/update fact source because it does not
carry per-track add timestamps.

Each saved-source-track item keeps generic canonical hints for title, artists,
NetEase album source ref/label, and duration when present.
...
Saved releases read `/album/sublist` with stable pagination, then fetch
`/album?id=<albumId>` best-effort. The album payload can populate
...
Followed artists read `/artist/sublist` with stable pagination. The provider
marks followed artists as newest-first through its area descriptor, but current
NetEase payloads do not expose an item-level followed timestamp, so the adapter
does not invent `providerAddedAt` for artist items.
...
- Library Import owns any Source Entity Store / Source Library writes derived
  from platform-library facts.
...
- platform-library preview/read behavior for saved tracks, saved releases, and
  followed artists.
- platform-library paged reads through `readPage(...)`.
...
- liked-playlist saved-track ordering and provider add-time extraction.
