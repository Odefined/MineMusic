...
| `LibraryImportMaterialStorePort` | Material Store | Source Entity Store, Source Library, and eager durable material binding state for imported source refs | `getSourceEntity`, `getSourceLibraryItem`, `listSourceLibraryItems` | `upsertSourceEntity`, `putSourceLibraryItem`, `getOrCreateBySourceRef` |
...
- Imported provider items enter Source Entity Store and Source Library first,
  then Library Import ensures a durable MaterialRecord exists for the imported
  `sourceRef`.
