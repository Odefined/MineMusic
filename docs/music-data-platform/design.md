...
| Library Catalog stage adapter | MDP-owned Stage Adapter boundary for the `library.catalog.*` public tool surface. | Exposes read-only list-scopes, browse, seed-sample, and summary tools over owner catalog projection plus search metadata; excludes provider scopes and raw catalog rows. |
