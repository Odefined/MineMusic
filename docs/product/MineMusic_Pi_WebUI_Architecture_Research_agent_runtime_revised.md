...
> 修订说明：本版在原报告基础上明确两次关键收束：第一，把原先容易被理解为“外部 Pi adapter 接入 MineMusic”的表述改为：**MineMusic 是 agent-native runtime；Agent Runtime 是 MineMusic 内部一等组件；Pi 只是 Agent Runtime 使用的 engine implementation**。第二，按 ADR-0030 将 **Agent Runtime** 与 **Session Context** 定为 formal top-level areas；本文后续不再把 Agent Runtime 塞进 Stage Core，也不再使用旧的 Workspace 命名作为正式所有权名称。因此，本文中的 `adapter` 应理解为 Agent Runtime 内部的反腐/engine 适配层，不是把 agent 放在 MineMusic 外部。
...
MineMusic Runtime
  ...
  │
  ├─ Session Context                 # selected object / intent / live context
...
4. **Session Context 与 Music Experience 必须一起承接 listening runtime。**
   当前 `src/music_experience/` 只有 `music.experience.present`。PRD 所需 selected object、latest intent、area revision、live queue/candidate/pacing context 应由 Session Context 拥有；durable Playback、Recommendation Batch、Radio outcome/history、play/open/skip events、feedback binding 和 external action intent 由 Music Experience 拥有。

5. **必须引入 server-authoritative Session Context，但不能让它变成新的全能 bounded context。**
   Session Context 保存会话 envelope、选中对象、最新显式意图、area-owned slice revision 和 live radio/listening context；agent run/message/work 归 Agent Runtime，durable playback/radio/recommendation 归 Music Experience，Library/identity 归 Music Data Platform，proposal/confirmation 归 Effect Boundary。
...
7. **Radio subagent 应是 Radio workflow 的执行参与者，不是第二套产品模型。**
   Session Context 保存 live motif、variations、intent epoch、queue/candidate/pacing context；Music Experience 保存 consequential radio history 和 recommendation outcomes；Pi Radio actor 在版本化 snapshot 上完成候选判断、解释和下一步建议，但不直接改 queue 或 durable state。
...
推荐采用**渐进式扩展**，不是重写：先建立 Session Context action/event/revision 基础，再接 Agent Runtime/Pi 主 agent，再接 Web shell，随后完成 Playback、Recommendation Batch 和 Radio。
...
| D3 | `ARCHITECTURE.md` | 十一个顶层 area、Server Host / Stage Core / Stage Interface / Agent Runtime / Session Context 所有权 |
...
| D9 | `docs/adr/0030-agent-runtime-and-session-context-are-top-level-areas.md` | Agent Runtime 与 Session Context 升为 formal top-level areas |
...
- Agent state 不等于 application state；Pi messages / agent run state、Session Context、Music Experience、Music Data Platform、Memory 必须分开。
- Agent 通过 snapshot / events / shared context 感知产品，通过 Stage Interface / commands / proposals 行动。
- Main Agent 与 Radio Agent 不共享无边界可变 prompt/messages；它们通过 typed actor messages、versioned Session Context snapshot 和 domain events 协作。
...
| Playback 常驻 | now playing、queue、controls、progress、volume、source、error | provider 有 `getPlayableLinks`；Music Experience 只有 `present` | 无 Session Context live playback/queue context、Music Experience durable playback outcome、browser controller、playback events |
...
| 用户显式操作优先 | revision/intent epoch、stale commit rejection | 当前 requestId/sessionId 已存在 | 无 Session Context revision 和 optimistic concurrency |
...
| Radio motif/variation | Session Context live radio state + Music Experience durable radio outcome | 无 | 缺失 |
...
| Session-level understanding | Session Context snapshot/context assembler | `StageToolContext` 只有调用级上下文 | 无 durable/reconnectable Session Context |
...
Session Context owns selected object, intent epoch, area revisions, and live
  queue/candidate/pacing context.
Music Experience owns durable playback/radio/recommendation outcomes and
  consequential listening history.
...
| Agent Runtime | actor lifecycle、context、stream、cancel、run persistence | Material/queue/radio 领域判断 |
| Session Context | selected object、intent epoch、area revisions、live queue/candidate/pacing context | agent run/message/work state、durable history |
...
新增 `AgentContextAssembler`，每次 turn 只注入：

- selected object public summary；
- current playback item/queue compact state；
- active radio motif/variations/direction/mode；
- latest recommendation batch summary；
- recent explicit actions；
- current proposals；
- latest intent epoch；
- relevant library scopes；
- speech preference（如“少说话”）；
- tool catalog。

不要将完整 DB row、provider payload、原始 event log 或全部 tool trace 塞进 LLM context。

Pi 的 `transformContext` 可用于压缩旧对话和注入 Session Context snapshot，但 MineMusic 应先生成稳定的 area-owned context DTO，再交给 Pi。
