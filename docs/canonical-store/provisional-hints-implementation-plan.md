...
Implement `docs/canonical-store/provisional-hints-implementation-plan.md`.

Goal: add platform-neutral provisional canonical hints for source-side recording
context. Do not extend `CanonicalRelation` for track position. NetEase should
best-effort fetch `/album?id=<albumId>` during saved-recording reads and add
`canonicalHints.trackPosition` when available. Library Import should preserve
provider hints in provenance and also record a Canonical Store
`source_recording_context` provisional hint for provisional recording records.

Start by reading:

- `AGENTS.md`
- `INDEX.md`
- `docs/canonical-store/provisional-hints-implementation-plan.md`
- `docs/mvp/interface-contracts.md`
- `docs/mvp/module-interfaces.md`
- `src/contracts/index.ts`
- `docs/platform-library-provider/design.md`
...
State-sync gate for the final report:

- `INDEX.md`: update only if this plan becomes a source-of-truth index entry or
  a new implementation doc needs indexing.
- `CURRENT_STATE.md`: update if code behavior changes.
- `ARCHITECTURE.md`: likely not needed unless module ownership changes.
- `PROGRESS.md`: update if implementation lands.
