...
| --- | --- | --- |
| Main turn harness | `Agent.prompt`, `Agent.continue`, `Agent.abort`, `Agent.waitForIdle`, lifecycle listeners, `state.messages`, `steer`, `followUp` from `dist/agent.js`. | Turn assembly, Agent Context rail injection, user-visible response policy, actor lifecycle ownership. |
...
| Prompt assembly harness | `loadPromptTemplates`, `loadSourcedPromptTemplates`, `formatPromptTemplateInvocation`, and argument substitution from `dist/harness/prompt-templates.*`. | MineMusic Agent Context rail shape, product prompt policy, per-actor prompt structure. |
