...
| `phase-21-postgres-background-work-localize-implementation-plan.md` | Active Phase 21 execution plan for Postgres runtime storage, Background Work, download helper extraction, and localize runtime wiring. |
| `phase-22-search-core-metadata-lookup-refactor-implementation-plan.md` | Active Phase 22 execution plan for Postgres-native Search Core metadata lookup over `search_metadata_documents`, result windows, provider/local reranking, and default `music.discovery.lookup` wiring. |
