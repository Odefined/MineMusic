...
| `phase-A-in-process-agent-native-loop-spec.md` | Phase A spec: in-process single-writer agent-native loop (A1 pi spine → A2 Session Context → A3 Music Experience queue/playback command → A4 end-to-end wiring), with owners, write boundary, guards, and PR split; PR A1a has landed the first `src/agent_runtime` pi spine and Stage tool bridge. |
