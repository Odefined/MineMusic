...
| `phase-A-in-process-agent-native-loop-spec.md` | Phase A spec: in-process single-writer agent-native loop (A1 pi spine → A2 Session Context → A3 Music Experience queue/playback command → A4 end-to-end wiring), with owners, write boundary, guards, and PR split; PR A1a/A1b/A2/A3 have landed the `src/agent_runtime` pi spine, Stage tool bridge, hook guards, minimal Workbench Interface read-model seam with Agent Runtime Session Context assembly, and Music Experience queue/playback truth plus Stage tools. |
