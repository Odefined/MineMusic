...
| `phase-24-collection-foundation.md` | Phase 24 decision record for the Collection fact family: product frame, identity, kind, ordering, soft-remove, projection integration, and slice skeleton. |
| `phase-24-collection-foundation-implementation-plan.md` | Phase 24 implementation plan carrying the four v1 slices as PR 24A/24B/24C/24D (24E deferred): writer → projection → catalog scope → tools, with per-PR files/reads/writes/guards/verification. |
| `phase-25-local-source-root-path-identity-implementation-plan.md` | Implemented Phase 25 execution plan replacing md5 local-source identity with Main Local Source Root / scan-root path identity, non-content-derived localize paths, and root/path storage guards. |
| `phase-26-local-source-scan-management.md` | Grilled Phase 26 planning spec for configured scan roots, local audio discovery, durable background scan management, trusted deletion reconciliation, and internal catalog integration. |
| `issue-115-effect-boundary-adr-0038-gate-contract-implementation-plan.md` | Planned issue #115 execution plan for migrating Effect Boundary gate policy from per-scenario durable-write booleans to ADR-0038 `impactClass × actorTrustBasis`, adding `ownerCurationWrite`, and landing the source-of-truth edit tightening input without implementing Proposal Unit persistence. |
| `active-tree-ast-import-graph-implementation-plan.md` | Planned execution plan for replacing brittle active-tree exact file-list guards with AST-backed import/export graph rules by owner area and `stage_adapter` boundary, while preserving stale-tree, artifact, tiny-root, and contract-DAG guards. |
| `mdp-runtime-module-ownership-redraw-implementation-plan.md` | Planned execution plan for ADR-0045: moving database lifecycle, schema composition, and cross-area capability construction out of the Music Data Platform runtime module, then optionally decomposing its internals into private builders. |
| `agent-native-workbench-roadmap.md` | Planning roadmap sequencing the foundation toward the Agent-Native Workbench PRD: Phase A in-process loop, Phase B Radio + concurrency, Phase C AG-UI Web boundary, with locked sequencing decisions. |
| `agent-context-engineering-spec.md` | Current Agent Runtime context-engineering authority: Actor Identity, Actor Instruction, Capability Context, Workspace Context, Invocation Context, Continuity Context, and Knowledge / Memory Context; Main and Radio share the Workspace Context assembly model rather than maintaining separate workspace-state renderers. |
| `phase-B-radio-plan-spec-prd-audit.md` | Audit finding and plan-rewrite input comparing the current Phase B Radio implementation plan against the Phase B spec, Agent Context spec, roadmap, PRD, and live code. Records missing posture, steering, queue correction, lifecycle, cascade, endurance, and PRD-coverage capability chains before further Phase B work. |
| `pi-harness-reuse-conclusions.md` | Source-read conclusion note for which MineMusic harness capabilities are needed by the Agent-Native Workbench roadmap and which `@earendil-works/pi-agent-core@0.80.2` / `pi-ai@0.80.2` root-exported helper surfaces can be reused through Agent Runtime facades. Records that full `AgentHarness` and Phase A skill runtime are out of scope. |
| `phase-A-in-process-agent-native-loop-spec.md` | Phase A spec: in-process single-writer agent-native loop (A1 pi spine → A2 pre-refactor context seam → A3 Music Experience queue/playback command → A4 end-to-end wiring), with owners, write boundary, guards, and PR split; PR A1a/A1b/A2/A3 have landed the `src/agent_runtime` pi spine, Stage tool bridge, hook guards, minimal Workbench Interface read-model seam with Agent Runtime pre-refactor context assembly, and Music Experience queue/playback truth plus Stage tools. Agent-context model superseded for new design by `agent-context-engineering-spec.md`; Session Context and the Workbench read-model seam are retired for new agent-context work. |
| `phase-B-radio-concurrency-spec.md` | Planned Phase B spec: Radio as a pi Agent loop with a deterministic supervisor (discrete re-prompted runs), per-area/per-concern Agent Work Basis OCC, the candidate→material→library three-layer queue model, and (PB8/ADR-0037) layered continuity — transcript soul over a radio-truth floor split into commanded direction and OCC-invisible evolved posture. |
| `phase-C-web-boundary-spec.md` | Planned Phase C spec: the AG-UI Web boundary. C1–C3 are the ADR-0036 seam resolutions; C4 covers Proposal Unit confirmation (auto-emitted Confirm card) and A2UI cards (ADR-0034). The Effect Boundary `ask` policy is settled by ADR-0038 (impact class × actor trust → allow / ask / raise-to-conversation; deny is a separate pre-gate). |
| `pi-agent-core-capability-audit-0.80.2.md` | First-hand capability audit of `@earendil-works/pi-agent-core@0.80.2` (read `.d.ts`/source + fresh runtime checks): settles the phase-A pi Capability Ledger and the PB8a gate. Key verdicts — low-level `Agent` is volatile (persistence/compaction harness-only), no subagent primitive, `state.messages` externally truncatable (PB8a passes), raw JSON Schema usable as `Tool.parameters`, DeepSeek via built-in `openai-completions`; dominant risk is version churn (pin + re-audit on bump). Evidence input to the engine-choice ADR. |
