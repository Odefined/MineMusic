...
| `pi-harness-reuse-conclusions.md` | Source-read conclusion note for which MineMusic harness capabilities are needed by the Agent-Native Workbench roadmap and which `@earendil-works/pi-agent-core@0.79.10` / `pi-ai@0.79.10` source surfaces can be reused through Agent Runtime facades. Records that full `AgentHarness` and Phase A skill runtime are out of scope. |
| `phase-A-in-process-agent-native-loop-spec.md` | Planned Phase A spec: in-process single-writer agent-native loop (A1 pi spine → A2 Session Context → A3 Music Experience queue/playback command → A4 end-to-end wiring), with owners, write boundary, guards, and PR split. |
