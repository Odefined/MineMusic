...
3. **Phase B PR1 / PR2 / PR3 are already LANDED** (PR1 `08588893`/PR#128, PR2 `6ee1c8e8`/PR#129, PR3 `d10eb9ad` + follow-ups `e006a562`/`95c60ba5`/`f846fc9a`/`382c9d03`/`cac18fe9`/`c5cadd8f`). `radio_run.ts` / `radio_supervisor.ts` / `agent_runtime_radio_module.ts` / `main_agent_session.ts` / `session_context.ts` all exist. The refactor **migrates landed code**, it does not create it. **Do not restructure PR3 in the plan** — see §6.
...
- Files: `src/agent_runtime/main_agent_session.ts`; **delete** `src/agent_runtime/session_context.ts`; `src/workbench_interface/read_model.ts`; `src/contracts/workbench_interface.ts`; `test/formal/agent-runtime-main-agent-session.test.ts`.
