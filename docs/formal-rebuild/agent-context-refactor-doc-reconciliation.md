...
- One **`ActorDefinition`** per actor: `runtimePolicy{actorKind,cascadePriority,additionalToolPreconditionBasis}` / `identity{role,job,persona}` / `instruction{responsibilities,operatingRules,prohibitions}` / `declaredWorkspaceSections` / `toolPack{stageToolNames}`. Identity and actor-specific execution policy are not scattered across server modules.
