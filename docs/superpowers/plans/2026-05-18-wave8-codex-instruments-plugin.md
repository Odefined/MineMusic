...
- Modify `src/stage_interface/index.ts` to list, dispatch, and enforce instrument tools.
...
- Create `test/stage_interface/stage-interface-dispatch.test.ts` additions for `stage.materials.prepare` and enforcement.
...
- Modify: `src/stage_interface/index.ts`
...
and that `createToolDispatch(...).call({ toolName: "stage.materials.prepare", ... })` returns materials prepared by Stage Modules. Add a second test where the session has `activeInstruments: ["other.instrument"]` and calling `music.material.resolve` returns `stage_interface.tool_not_found`.
...
Add `createMineMusicStageCoreWithSourceProvider(...)` or equivalent. It should share the existing composition path, seed optional canonical records, and register the supplied source provider.
...
- `minemusic.stage.materials.prepare` delegates through Stage Interface and returns gated material.
