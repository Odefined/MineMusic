...
Phase A projection note had been silently rewritten.
...
Experience owning command and let Session Context render the queue it reads.
That is a real product/architecture decision, not a clarification of the
...
Workbench Interface and Agent Runtime Session Context may expose/render the full
queue because the queue itself is now bounded. They must not hide an unbounded
product queue behind prompt-only truncation.
...
- **Prompt-only truncation in Session Context.** Rejected because it hides an
...
- **Workbench-only bounded projection (`queueHead` / `queueTail` /
  ...
  Session Context and the product queue has no reason to be unlimited in the
...
- Session Context may continue to render the full queue, but this remains safe
  only while the Music Experience queue bound is enforced by the owning command.
