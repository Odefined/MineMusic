...
| Actor Identity | Structured actor-facing role, job, and persona, sourced from `ActorDefinition.identity`. | First shape: `role`, `job`, and `persona`. It is not `ActorDefinition.name`, a raw prompt paragraph, operational guidance, or a place for internal data-pipeline terms. |
