...
| `library.catalog.*` | Public Agent Protocol tool family for read-only catalog inspection: `list_scopes`, `browse`, `sample`, and `summary`. | Reads owner-visible catalog membership through Music Data Platform catalog projections; public item descriptions and summary signal fields come from Material Projection, not search metadata. `list_scopes` excludes provider and `all`; `browse` pages compact public items; `sample` is deterministic by caller seed; `summary` returns timeline evidence, kind-separated concentration signals, and library-baseline membership signals. Not raw rows, provider search, recommendation output, Memory, or relation editing. |
