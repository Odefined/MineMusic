...
| Agent Context Engineering | Agent Runtime-owned assembly model for embedded-agent model context. | Splits context into Actor Instruction, Capability Context, Workspace Context, Invocation Context, Continuity Context, and Knowledge / Memory Context. It is not a single prompt blob, a top-level area, Workbench Interface state ownership, Stage Interface tool ownership, or pi transcript storage. |
...
| Continuity Context | Conversation and execution continuity carried through transcript, compaction, and retained history. | Helps an actor continue prior work. It is not current workspace truth; state changes must re-enter Workspace Context through owning facts/projections, not transcript scraping. |
| Knowledge / Memory Context | Retrieved knowledge, taste hints, durable taste memory, and reference material used for reasoning. | Phase B starts with `userTasteHint` from the same underlying catalog-summary capability as `library.catalog.summary`, through an internal context provider rather than a Stage tool self-call; that hint is not durable Memory. Future inputs may include Memory, Knowledge, Handbook, docs, and search/reference material when loaded. It may influence future choices but does not rewrite current Workspace Context facts. |
