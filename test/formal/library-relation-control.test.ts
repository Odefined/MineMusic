...
import {
  ...
  type LibraryRelationControlPort,
...
assert.deepEqual(
  relationDescriptors.map((descriptor) => descriptor.name),
  [
...
assert.deepEqual(
  libraryRelationGetDescriptor.errors.map((error) => error.code),
  ["invalid_input", "item_not_found", "owner_scope_unsupported"],
);
...
for (const descriptor of relationDescriptors.slice(1)) {
  ...
  assert.deepEqual(
    descriptor.errors.map((error) => error.code),
    ["invalid_input", "item_not_found", "owner_scope_unsupported", "item_not_writable"],
  );
}
...
function assertDeclaredError(
  result: Awaited<ReturnType<ReturnType<typeof interfaceFor>["dispatch"]>>,
  code: string,
): void {
  assert.equal(result.ok, false);
