...
import { DEFAULT_RETRIEVAL_LIMIT, MAX_RETRIEVAL_POOL_GROUP_SIZE, MAX_RETRIEVAL_POOL_TOTAL, MusicIntelligenceError, createRetrievalQueryService, isMusicIntelligenceError, type CreateRetrievalQueryServiceInput, type RetrievalEffectiveQuery, type RetrievalPool, type RetrievalPoolFilter, type RetrievalQueryHit, type RetrievalQueryInput, type RetrievalQueryMaterialCandidateHit, type RetrievalQueryMaterialHit, type RetrievalQueryResult, type RetrievalQueryService, } from "../../src/music_intelligence/index.js";
import { DEFAULT_OWNER_SCOPE, createMaterialTextProjectionCommands, createMusicDataPlatformRetrievalWorkspace, createMusicDataPlatformRetrievalReadPort, createOwnerCatalogProjectionCommands, createSourceLibraryRef, musicDataPlatformIdentitySchema, musicDataPlatformMaterialTextProjectionSchema, musicDataPlatformOwnerCatalogEntriesSchema, musicDataPlatformOwnerCatalogViewSchema, musicDataPlatformOwnerRelationSchema, musicDataPlatformProjectionMaintenanceSchema, musicDataPlatformRetrievalResultSetSchema, musicDataPlatformSourceLibrarySchema, } from "../../src/music_data_platform/index.js";
import type { MusicDataPlatformRetrievalWorkspace, MusicDataPlatformRetrievalMaterialRow, MusicDataPlatformRetrievalReadPort, MusicDataPlatformRetrievalSearchInput, MusicDataPlatformRetrievalSearchPage, RetrievalFreshness, RetrievalReadCursorPosition, } from "../../src/music_data_platform/index.js";
...
export type _createRetrievalQueryServiceInputShape = Expect<Equal<keyof CreateRetrievalQueryServiceInput, "readPort" | "mixedRetrievalWorkspace" | "providerSearch">>;
export type _retrievalQueryServiceShape = Expect<Equal<keyof RetrievalQueryService, "query">>;
export type _retrievalQueryServiceQueryReturnShape = Expect<Equal<ReturnType<RetrievalQueryService["query"]>, Promise<RetrievalQueryResult>>>;
export type _retrievalQueryInputShape = Expect<Equal<keyof RetrievalQueryInput, "ownerScope" | "text" | "materialKind" | "pools" | "order" | "limit" | "cursor" | "sessionId">>;
export type _retrievalEffectiveQueryShape = Expect<Equal<keyof RetrievalEffectiveQuery, "ownerScope" | "text" | "materialKind" | "pools" | "order">>;
export type _retrievalQueryResultShape = Expect<Equal<keyof RetrievalQueryResult, "query" | "basis" | "hits" | "page" | "freshness">>;
export type _retrievalQueryHitShape = Expect<Equal<keyof RetrievalQueryHit, "kind" | "display" | "rankScore" | "matchedText" | "pools" | "basis">>;
export type _retrievalQueryMaterialHitShape = Expect<Equal<keyof RetrievalQueryMaterialHit, "kind" | "materialRef" | "materialKind" | "display" | "rankScore" | "matchedText" | "pools" | "basis">>;
export type _retrievalQueryMaterialCandidateHitShape = Expect<Equal<keyof RetrievalQueryMaterialCandidateHit, "kind" | "materialCandidateRef" | "display" | "rankScore" | "matchedText" | "pools" | "basis">>;
export type _retrievalPoolFilterShape = Expect<Equal<keyof RetrievalPoolFilter, "allOf" | "anyOf" | "noneOf">>;
export type _retrievalPoolShape = Expect<Equal<RetrievalPool, {
    kind: "local_catalog";
} | {
    kind: "source_library";
    ref: Ref;
...
await assertMusicIntelligenceError(() => normalizedEmptyHarness.service.query({
    text: "   ",
    order: "text_relevance",
...
await assertMusicIntelligenceError(() => droppedTextHarness.service.query({
    text: "--- !!!",
    order: "text_relevance",
}), "music_intelligence.retrieval_query_invalid");
...
await assertMusicIntelligenceError(() => validationHarness.service.query({ ownerScope: "other" }), "music_intelligence.retrieval_query_invalid");
await assertMusicIntelligenceError(() => validationHarness.service.query({ limit: 0 }), "music_intelligence.retrieval_query_invalid");
await assertMusicIntelligenceError(() => validationHarness.service.query({ limit: 101 }), "music_intelligence.retrieval_query_invalid");
await assertMusicIntelligenceError(() => validationHarness.service.query({ limit: 1.5 }), "music_intelligence.retrieval_query_invalid");
await assertMusicIntelligenceError(() => validationHarness.service.query({
...
await assertMusicIntelligenceError(() => poolHarness.service.query({
    poolFilter: {
        allOf: [libraryPoolA],
    },
} as unknown as RetrievalQueryInput), "music_intelligence.retrieval_query_invalid");
await assertMusicIntelligenceError(() => poolHarness.service.query({
    pools: {
        allOf: Array.from({ length: MAX_RETRIEVAL_POOL_GROUP_SIZE + 1 }, (_, index) => sourceLibraryPool(sourceLibraryRef("saved_source_track", `cap_group_${index}`))),
    },
}), "music_intelligence.retrieval_query_invalid");
await assertMusicIntelligenceError(() => poolHarness.service.query({
    pools: {
        allOf: Array.from({ length: MAX_RETRIEVAL_POOL_GROUP_SIZE }, (_, index) => sourceLibraryPool(sourceLibraryRef("saved_source_track", `cap_total_all_${index}`))),
        anyOf: Array.from({ length: MAX_RETRIEVAL_POOL_GROUP_SIZE }, (_, index) => sourceLibraryPool(sourceLibraryRef("saved_source_album", `cap_total_any_${index}`))),
        noneOf: [
...
const fixtureMixedDatabase = await initializedDatabase();
...
await fixtureMixedDatabase.close();
{
    ...
    assert.equal(mixedWorkspace.calls[0]?.queryFingerprint, mixedWorkspace.calls[1]?.queryFingerprint);
}
{
    const mixedWorkspace = createMixedWorkspaceHarness();
    ...
    for (const materialKind of ["release", "work"] as const) {
        await assertMusicIntelligenceError(() => service.query({
            text: "Kind Map",
            materialKind,
            pools: {
                anyOf: [{
...
}
{
    ...
    await assertMusicIntelligenceError(() => failedService.query({
    ...
    await assertMusicIntelligenceError(() => unavailableService.query({
        text: "Provider Failure",
        pools: {
            anyOf: [{
                    kind: "provider_search",
    ...
    await assertMusicIntelligenceError(() => invalidResultService.query({
        text: "Provider Failure",
        pools: {
            anyOf: [{
                    kind: "provider_search",
    ...
    await assertMusicIntelligenceError(() => invalidTargetKindService.query({
        text: "Provider Failure",
        materialKind: "album",
        pools: {
            anyOf: [{
    ...
    await assertMusicIntelligenceError(() => invalidOptionalSourceFieldService.query({
        text: "Provider Failure",
        pools: {
            anyOf: [{
                    kind: "provider_search",
...
// fixture helpers set them equal, so this case is the one that guards real providers.
{
    ...
    assert.equal(refKey(accepted.sourceEntity.sourceRef), refKey(sourceRefForProvider("netease", "track", "ncm_12345")));
...
for (const [status, code] of [
    ...
    await assertMusicIntelligenceError(() => statusService.query({
        text: "fixture alpha",
        pools: {
            anyOf: [{
                    kind: "provider_search",
...
assert.deepEqual(materialHitRefKeys(secondCursorResult.hits), [refKey(materialRef("recording", "m_page_2"))]);
await assertMusicIntelligenceError(() => cursorHarness.service.query({
    order: "stable",
    materialKind: "album",
    cursor: nextCursor,
...
assert.deepEqual(materialHitRefKeys(stableRankResult.hits), [
...
await assertMusicIntelligenceError(() => invalidRankHarness.service.query({
    text: "plain",
...
await assertMusicIntelligenceError(() => missingTextFieldsHarness.service.query({
    text: "plain",
}), "music_intelligence.retrieval_result_invalid");
...
await assertMusicIntelligenceError(() => missingTextTokensHarness.service.query({
    text: "plain",
}), "music_intelligence.retrieval_result_invalid");
...
await assertMusicIntelligenceError(() => missingTextTokenCountHarness.service.query({
    text: "plain",
}), "music_intelligence.retrieval_result_invalid");
...
await assertMusicIntelligenceError(() => zeroTextTokenCountHarness.service.query({
    text: "plain",
}), "music_intelligence.retrieval_result_invalid");
const integrationDatabase = await initializedDatabase();
const integrationLibraryRef = mdpSourceLibraryRef(DEFAULT_OWNER_SCOPE, "7601", "saved_source_track");
...
await integrationDatabase.transaction(async (db) => {
    const identity = createIdentityTestCommands(db, "2026-06-14T06:00:00.000Z");
    const libraries = createSourceLibraryRepositories({ db });
    await bindSourceToMaterial(identity, integrationAccentSource, integrationAccentMaterialRef);
    await bindSourceToMaterial(identity, integrationLiltSourceOne, integrationLiltMaterialRefOne);
    await bindSourceToMaterial(identity, integrationLiltSourceTwo, integrationLiltMaterialRefTwo);
    await upsertActualLibrary(libraries, integrationLibraryRef, DEFAULT_OWNER_SCOPE, "7601", "saved_source_track");
    await upsertActualLibraryItem(libraries, integrationLibraryRef, integrationAccentSource.sourceRef, "2026-06-14T06:01:00.000Z");
    await upsertActualLibraryItem(libraries, integrationLibraryRef, integrationLiltSourceOne.sourceRef, "2026-06-14T06:02:00.000Z");
    await upsertActualLibraryItem(libraries, integrationLibraryRef, integrationLiltSourceTwo.sourceRef, "2026-06-14T06:03:00.000Z");
    ...
    await textCommands.rebuildMaterialTextDocument({ materialRef: integrationAccentMaterialRef });
    await textCommands.rebuildMaterialTextDocument({ materialRef: integrationLiltMaterialRefOne });
    await textCommands.rebuildMaterialTextDocument({ materialRef: integrationLiltMaterialRefTwo });
});
...
assert.deepEqual(materialHitRefKeys(integrationAccentResult.hits), [refKey(integrationAccentMaterialRef)]);
...
assert.notEqual(refKey(materialHitRef(integrationTextPageOne.hits[0])), refKey(materialHitRef(integrationTextPageTwo.hits[0])));
assert.deepEqual([
...
assert.deepEqual(materialHitRefKeys(integrationDroppedTextResult.hits), [
    refKey(integrationLiltMaterialRefTwo),
    refKey(integrationLiltMaterialRefOne),
]);
await assertMusicIntelligenceError(() => integrationService.query({
    text: "--- !!!",
    order: "text_relevance",
}), "music_intelligence.retrieval_query_invalid");
...
function createReadPortHarness(pages: readonly (MusicDataPlatformRetrievalSearchPage & {
    ...
    const readPort = {
        async searchOwnerCatalogMaterials(input) {
        ...
        async getRetrievalFreshness(input) {
...
function createMixedWorkspaceHarness(): {
    ...
    return {
        calls,
        workspace: {
            async searchMixedResultSet(input) {
                calls.push(input);
                return {
                    status: "ok",
...
async function assertMusicIntelligenceError(run: () => unknown | Promise<unknown>, code: MusicIntelligenceError["code"]): Promise<void> {
    let thrown: unknown;
    try {
        await run();
    }
...
async function upsertActualLibraryItem(libraries: ReturnType<typeof createSourceLibraryRepositories>, libraryRef: Ref, sourceRef: Ref, addedAt: string): Promise<void> {
...
function mdpSourceLibraryRef(ownerScope: string, providerAccountId: string, libraryKind: "saved_source_track" | "saved_source_album" | "followed_source_artist"): Ref {
