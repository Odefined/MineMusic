...
export type _mineMusicRuntimeProjectionMaintenanceShape = Expect<Equal<NonNullable<MineMusicRuntimeConfig["projectionMaintenance"]>, {
    enabled?: boolean;
    intervalMs?: number;
    batchLimit?: number;
}>>;
export type _projectionMaintenanceSchedulerConfigShape = Expect<Equal<keyof ProjectionMaintenanceSchedulerConfig, "enabled" | "intervalMs" | "batchLimit">>;
export type _projectionMaintenanceSchedulerDependenciesShape = Expect<Equal<keyof ProjectionMaintenanceSchedulerDependencies, "now" | "setTimeout" | "clearTimeout">>;
export type _createProjectionMaintenanceSchedulerInputShape = Expect<Equal<keyof CreateProjectionMaintenanceSchedulerInput, "database" | "config" | "dependencies">>;
export type _projectionMaintenanceSchedulerSnapshotShape = Expect<Equal<keyof ProjectionMaintenanceSchedulerSnapshot, "enabled" | "running" | "lastRunAt" | "lastSummary" | "lastError">>;
export type _projectionMaintenanceSchedulerShape = Expect<Equal<keyof ProjectionMaintenanceScheduler, "start" | "stop" | "snapshot">>;
assert.throws(() => createProjectionMaintenanceScheduler({
...
}
{
    ...
    await markMaterialTextTargetDirty(database, "material-1");
    const scheduler = createProjectionMaintenanceScheduler({
        database,
        config: {
            enabled: false,
...
}
{
    ...
    const database = await initializedDatabase();
    await markMaterialTextTargetDirty(database, "material-1");
    await markMaterialTextTargetDirty(database, "material-2");
    const scheduler = createProjectionMaintenanceScheduler({
        database,
        config: {
            batchLimit: 1,
    ...
    await scheduler.start();
    assert.deepEqual(timers.activeDelays(), [0]);
    assert.equal((await listPendingProjectionTargets(database)).length, 2);
    assert.deepEqual(scheduler.snapshot(), {
        enabled: true,
        running: false,
    });
...
async function waitForSchedulerIdle(scheduler: ProjectionMaintenanceScheduler): Promise<void> {
    for (let attempt = 0; attempt < 10000; attempt += 1) {
        if (!scheduler.snapshot().running) {
            return;
        }
    ...
    throw new Error("Expected projection maintenance scheduler to become idle.");
}
