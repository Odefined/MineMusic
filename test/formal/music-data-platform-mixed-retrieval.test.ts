...
import { DEFAULT_OWNER_SCOPE, createMaterialTextProjectionCommands, createMusicDataPlatformRetrievalWorkspace, createOwnerCatalogProjectionCommands, createProviderMaterialCandidateRef, createSourceLibraryRef, musicDataPlatformIdentitySchema, musicDataPlatformMaterialTextProjectionSchema, musicDataPlatformOwnerCatalogEntriesSchema, musicDataPlatformOwnerCatalogViewSchema, musicDataPlatformOwnerRelationSchema, musicDataPlatformProjectionMaintenanceSchema, musicDataPlatformRetrievalResultSetSchema, musicDataPlatformSourceLibrarySchema, type MusicDataPlatformMixedRetrievalRow, } from "../../src/music_data_platform/index.js";
...
await mixedDatabase.transaction(async (db) => {
    ...
    await upsertLibrary(libraries, mixedLibraryRef, "mixed");
    for (let index = 0; index < 11; index += 1) {
        const source = sourceTrack(`local_${index}`, `alpha local ${index}`);
        const nextMaterialRef = materialRef("recording", `m_local_${index}`);
        await bindSourceToMaterial(identity, source, nextMaterialRef);
        await upsertLibraryItem(libraries, mixedLibraryRef, source.sourceRef, `2026-06-15T09:${String(index).padStart(2, "0")}:00.000Z`);
    ...
    await bindSourceToMaterial(identity, mixedKnownSource, mixedKnownMaterialRef);
    await createMaterialTextProjectionCommands({
        db,
...
const mixedPageOne = await mixedWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "alpha beta",
    durablePoolFilter: {
...
assert.equal(mixedPageOne.rows[0]?.kind === "material"
    ...
    : undefined, refKey(mixedKnownMaterialRef));
...
const storedMixedSet = await createRetrievalResultSetRecords({
...
const mixedRows = await createRetrievalResultSetRecords({
    db: mixedDatabase.context(),
}).resultRows.listForResultSet({
    resultSetId: mixedPageOne.resultSetId,
});
assert.equal(mixedRows.length, 12);
assert.equal(mixedRows.filter((row) => row.materialRefKey === refKey(mixedKnownMaterialRef)).length, 1);
assert.equal(mixedRows.filter((row) => row.materialCandidateRefKey === mixedCandidateRefKey).length, 1);
assert.deepEqual(mixedRows.slice(0, 2).map((row) => row.rowKind), ["material", "material_candidate"]);
const mixedPageTwo = await mixedWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "alpha beta",
    includeLocalCatalog: false,
    order: "text_relevance",
...
assert.equal(mixedPageTwo.rows[0]?.kind === "material_candidate"
    ...
    : undefined, mixedCandidateRefKey);
...
assert.deepEqual(await mixedWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "alpha beta",
    includeLocalCatalog: false,
    order: "text_relevance",
...
const providerOnlyPage = await mixedWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "alpha",
    includeLocalCatalog: false,
    order: "text_relevance",
...
assert.deepEqual(providerOnlyPage.status === "ok"
    ? providerOnlyPage.rows.map((row) => row.kind)
    : [], ["material_candidate"]);
const missingTextDatabase = await initializedDatabase();
...
await missingTextDatabase.transaction(async (db) => {
    const identity = createIdentityTestCommands(db, "2026-06-15T10:03:00.000Z");
    await bindSourceToMaterial(identity, missingTextSource, missingTextMaterialRef);
});
const missingTextPage = await missingTextWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
...
assert.deepEqual(missingTextPage.status === "ok"
    ? missingTextPage.rows.map((row) => row.kind)
    : [], []);
assert.equal(await createRetrievalResultSetRecords({
...
const blockedPage = await blockedWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "blocked alpha",
    includeLocalCatalog: false,
    order: "text_relevance",
...
assert.deepEqual(blockedPage.status === "ok"
    ? blockedPage.rows.map((row) => row.kind)
    : [], ["material_candidate"]);
const expiryDatabase = await initializedDatabase();
...
const expiryPage = await expiryWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "expiry alpha",
    includeLocalCatalog: false,
    order: "text_relevance",
...
assert.deepEqual(await expiryWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "expiry alpha",
    includeLocalCatalog: false,
    order: "text_relevance",
...
const candidateExpiryDatabase = await initializedDatabase();
...
const candidateExpiryPage = await candidateExpiryWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
...
await candidateExpiryDatabase.context().run(`
    UPDATE material_candidate_cache
    SET expires_at = ?
  `, ["2026-06-15T13:01:00.000Z"]);
assert.deepEqual(await candidateExpiryWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "candidate expiry",
    includeLocalCatalog: false,
    order: "text_relevance",
...
const mergePageOne = await mergeWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "alpha beta",
    includeLocalCatalog: false,
    order: "text_relevance",
...
const mergePageTwo = await mergeWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "alpha beta",
    includeLocalCatalog: false,
    order: "text_relevance",
...
assert.notEqual(mixedRowKey(mergePageOne.rows[0]), mixedRowKey(mergePageTwo.rows[0]));
const providerDisplayDatabase = await initializedDatabase();
const providerDisplayWorkspace = createMusicDataPlatformRetrievalWorkspace({
    database: providerDisplayDatabase,
});
const providerDisplayPage = await providerDisplayWorkspace.searchMixedResultSet({
    ownerScope: DEFAULT_OWNER_SCOPE,
    text: "mili",
    includeLocalCatalog: false,
    order: "text_relevance",
...
await missingTextDatabase.close();
await blockedDatabase.close();
await expiryDatabase.close();
await candidateExpiryDatabase.close();
await mergeDatabase.close();
await providerDisplayDatabase.close();
...
function providerCandidate(sourceEntity: SourceEntity, providerScore?: number): ProviderMaterialCandidate {
    return {
        sourceEntity,
