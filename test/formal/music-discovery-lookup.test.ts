...
assert.deepEqual(musicDiscoveryLookupDescriptor.errors.map((error) => error.code), [
    "invalid_input",
    "invalid_cursor",
    "unknown_scope",
...
const plainRetrievalThrowResult = await lookupInterfaceForThrownRetrievalError(new Error("Retrieval query service is not initialized.")).dispatch(testStageToolContext({
    mintedAnchors: [],
}), {
    toolName: "music.discovery.lookup",
    payload: {
...
const retrievalResultInvalidResult = await lookupInterfaceForThrownRetrievalError(new MusicIntelligenceError({
...
const providerSearchPoolInvalidResult = await lookupInterfaceForThrownRetrievalError(new MusicIntelligenceError({
...
const cursorMismatchResult = await lookupInterfaceForThrownRetrievalError(new MusicIntelligenceError({
...
assert.equal((musicDiscoveryLookupDescriptor.inputSchema as {
    type?: unknown;
}).type, "object");
...
function testLookupCursorStore(input: {
    ...
    return {
        ...
        async resolve(resolveInput) {
            assertComparableTimestamp(now);
            const row = await rows.get(resolveInput.cursorId);
...
function assertPublicLookupOutputIsVeiled(output: MusicDiscoveryLookupOutput): void {
    ...
    for (const forbidden of [
        ...
        assert.equal(text.includes(forbidden), false, `lookup output leaked internal token '${forbidden}'`);
    }
}
