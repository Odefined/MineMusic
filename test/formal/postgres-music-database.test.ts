...
import {
  ...
  type PostgresMusicDatabaseContext,
...
assertDatabaseError(
  () => PostgresMusicDatabase.open({ connectionString: "" }),
...
await assertDatabaseErrorAsync(
  () => preInitDatabase.transaction(async () => undefined),
...
assert.deepEqual(
  (await context.all<{ label: string }>(
    "SELECT label FROM first_schema WHERE label > ? ORDER BY label",
    ["a"],
  )).map((row) => row.label),
  ["alpha", "beta"],
);
assert.equal(
  (await context.get<{ label: string }>(
    "SELECT label FROM first_schema WHERE label = ?",
    ["alpha"],
  ))?.label,
  "alpha",
);
...
const storedBlob = await context.get<{ payload: Uint8Array }>("SELECT payload FROM blob_schema");
...
assert.equal(
  (await context.get<{ count: string }>(
    "SELECT COUNT(*) AS count FROM first_schema WHERE label = ?",
    ["committed"],
  ))?.count,
  "1",
);
...
await orderedDatabase.transaction(async () => {
  await assertDatabaseErrorAsync(
    () => orderedDatabase.transaction(async () => undefined),
...
await assertDatabaseErrorAsync(
  () => capturedStaleContext.run("INSERT INTO first_schema (label) VALUES (?)", ["stale"]),
...
await assertDatabaseErrorAsync(
  () => failingDatabase.transaction(async () => undefined),
  "storage.database_initialization_failed",
);
await assertDatabaseErrorAsync(
  () => failingDatabase.initialize(),
...
function schema(
  id: string,
  order: string[],
  apply: (context: PostgresMusicDatabaseContext) => Promise<void>,
...
async function assertDatabaseErrorAsync(
  operation: () => Promise<unknown>,
  code: MusicDatabaseError["code"],
): Promise<void> {
  await assert.rejects(
    operation,
    (error) => isMusicDatabaseError(error) && error.code === code,
