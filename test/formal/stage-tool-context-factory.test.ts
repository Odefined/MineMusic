...
// shipped handler reads it), so the context keeps the conservative default.
{
  ...
  const ctx = factory.createToolContext({
  ...
  assert.equal(
    await ctx.providerAvailability.isProviderAvailable({ providerId: "netease", ownerScope: "local" }),
    false,
  );

  const controller = new AbortController();
  const ctxWithAbort = factory.createToolContext({
    sessionId: "s",
    requestId: "r",
...
// a real conservative gate + audit.
{
  ...
  const ctx = factory.createToolContext({ sessionId: "s", requestId: "r" });
...
// produces a context whose handleMinting is the real registry-backed port.
{
  const host = createServerHost({
    config: { projectionMaintenance: { enabled: false } },
  });
  ...
  if (factory !== undefined) {
    const ctx = factory.createToolContext({ sessionId: "host-session", requestId: "host-request" });
    assert.equal(ctx.ownerScope, "local");
