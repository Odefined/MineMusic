...
import {
  ...
  type McpStdioTransportPorts,
} from "../../src/server/transports/mcp_stdio_driver.js";
import {
  errorResponse,
  parseJsonRpcLine,
  resultResponse,
...
} from "../../src/server/transports/mcp_framing.js";
import {
  deriveMcpAnnotations,
  renderMcpTool,
  stitchToolDescription,
} from "../../src/server/transports/mcp_rendering.js";
...
const readOnlyTestDescriptor: ToolDeclaration = {
  ...
  resultSummary: (result) => `ok=${(result as { ok?: boolean }).ok ?? false}`,
};
...
// a resultSummary that leaks an internal anchor is scrubbed to the fallback
{
  ...
  if (outcome.kind === "toolResult") {
    assert.equal(
      ...
      "Tool 'stage.test.ping' returned a result.",
    );
  }
}
...
// an error message that leaks an anchor is scrubbed to the code-only line.
{
  ...
  if (outcome.kind === "toolResult") {
    assert.equal(
      outcome.result.content[0]?.text,
      "Tool 'stage.test.ping' reported error 'invalid_input'.",
    );
  }
}
...
): { io: McpStdioTransportIo; written: string[]; errors: string[]; closeEof: () => void } {
