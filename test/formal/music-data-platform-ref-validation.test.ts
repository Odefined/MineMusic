...
assert.throws(() => assertMusicDataPlatformRefSafe({
    ref: {
        namespace: "",
        kind: "recording",
        id: "m_invalid",
...
assert.throws(() => musicDataPlatformRefKey({
    ref: {
        namespace: "material",
        kind: "recording",
        id: "m:invalid",
...
assert.throws(() => assertMusicDataPlatformPublicRefKey({
    refKey: "material:recording:m:invalid",
    fieldName: "cursorPosition.materialRefKey",
    code: "music_data.retrieval_read_invalid",
}), (error: unknown) => isMusicDataPlatformError(error) &&
    error.code === "music_data.retrieval_read_invalid");
assert.throws(() => assertOwnerScope("local:bad"), (error: unknown) => isMusicDataPlatformError(error) &&
    error.code === "music_data.owner_scope_invalid");
assert.throws(() => assertMaterialRef({
    namespace: "material",
    kind: "recording",
    id: "m:bad",
}), (error: unknown) => isMusicDataPlatformError(error) &&
    error.code === "music_data.material_ref_invalid");
assert.throws(() => assertSourceLibraryRef({
...
assert.throws(() => assertOwnerMaterialRelationRef({
