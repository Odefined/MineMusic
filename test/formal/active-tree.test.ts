...
for (const publicBarrel of [
    ...
    for (const forbiddenExport of [
        ...
        if (exportsName(text, forbiddenExport)) {
            musicIntelligencePublicBarrelExportFailures.push(`${publicBarrel} exports Retrieval internal '${forbiddenExport}'`);
        }
    }
}
assert.deepEqual(musicIntelligencePublicBarrelExportFailures, [], "Music Intelligence public barrels must not expose opaque cursor or query-normalization internals");
...
for (const file of activeFiles) {
    ...
    for (const name of declared) {
        if (!veilDenylistNames.has(name)) {
            veilDenylistDriftFailures.push(`${relative(repositoryRoot, file)} declares internal-anchor field '${name}' that is not in INTERNAL_ANCHOR_PROPERTY_NAMES`);
        }
    }
}
...
const transportImportSpecifierPattern = /\bfrom\s+["']([^"']+)["']|\bimport\s*\(\s*["']([^"']+)["']\s*\)|\bimport\s+["']([^"']+)["']/gu;
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/server/transports"))) {
    ...
    for (const match of text.matchAll(transportImportSpecifierPattern)) {
        ...
        if (root !== undefined && transportForbiddenRoots.has(root)) {
            transportImportFailures.push(`${relativeFile} imports forbidden transport dependency '${root}' via '${specifier}'`);
        }
    }
}
assert.deepEqual(transportImportFailures, [], "MCP stdio transport must import only contracts, Stage Interface veil/factory surfaces, and its own pure modules");
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/extension/plugins"))) {
    ...
    for (const forbiddenImport of forbiddenRelativeRootImportHits(text, [
        ...
        sourceProviderPluginImportFailures.push(`${relative(repositoryRoot, file)} imports forbidden provider implementation dependency '${forbiddenImport}'`);
    }
}
assert.deepEqual(sourceProviderPluginImportFailures, [], "Source provider plugins must not import Music Data Platform write/storage modules or presentation/runtime boundaries");
assert.deepEqual(forbiddenRelativeRootImportHits(`
      import type { MusicDatabase } from "../../../storage/index.js";
      import type { SourceEntity } from "../../contracts/music_data_platform.js";
    `, ["storage"]), ["../../../storage/index.js"], "Source provider plugin guard must catch forbidden imports from nested plugin directories");
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/effect_boundary"))) {
    ...
    for (const forbiddenImport of [
        ...
        if (text.includes(`from "${forbiddenImport}`) ||
            text.includes(`from '${forbiddenImport}`)) {
            effectBoundaryImportFailures.push(`${relative(repositoryRoot, file)} imports forbidden Effect Boundary dependency '${forbiddenImport}'`);
        }
    }
}
assert.deepEqual(effectBoundaryImportFailures, [], "Effect Boundary gate implementation must depend only on contracts and local policy helpers");
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/music_data_platform"))) {
    ...
    for (const rawLegacyStorageToken of ["Database" + "Sync", "node" + ":" + "sql" + "ite"]) {
        if (!text.includes(rawLegacyStorageToken)) {
            continue;
        }
        musicDataPlatformImportFailures.push(`${relative(repositoryRoot, file)} mentions raw legacy storage primitives`);
    }
    if (text.includes("MusicCard")) {
        musicDataPlatformImportFailures.push(`${relative(repositoryRoot, file)} mentions forbidden Stage Interface output type 'MusicCard'`);
    }
}
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/music_intelligence"))) {
    ...
    for (const forbiddenImport of [
        ...
        if (text.includes(`from "${forbiddenImport}`) ||
            text.includes(`from '${forbiddenImport}`)) {
            if (relativeFile === "src/music_intelligence/stage_adapter/index.ts" &&
                forbiddenImport === "../../stage_core/" &&
                (text.includes('from "../../stage_core/runtime_module.js"') ||
                    text.includes("from '../../stage_core/runtime_module.js'"))) {
                continue;
            }
            musicIntelligenceImportFailures.push(`${relativeFile} imports forbidden Music Intelligence dependency '${forbiddenImport}'`);
        }
    }
    for (const mdpImport of [
        ...
        if ((text.includes(`from "${mdpImport}`) || text.includes(`from '${mdpImport}`)) &&
            !(text.includes(`from "${mdpImport}index.js"`) || text.includes(`from '${mdpImport}index.js'`))) {
            musicIntelligenceImportFailures.push(`${relativeFile} imports Music Data Platform internals instead of the public retrieval read-port boundary`);
        }
    }
    for (const importedName of musicDataPlatformIndexImportNames(text)) {
        if (!musicIntelligenceAllowedMdpImports.has(importedName)) {
            musicIntelligenceImportFailures.push(`${relativeFile} imports disallowed Music Data Platform index symbol '${importedName}'`);
        }
    }
    if (hasMusicDataPlatformIndexNamespaceImport(text)) {
        musicIntelligenceImportFailures.push(`${relativeFile} imports Music Data Platform index through a namespace import`);
    }
    ...
    for (const sqlToken of [
        ...
        if (text.includes(sqlToken)) {
            musicIntelligenceImportFailures.push(`${relativeFile} contains SQL token '${sqlToken.trim()}' outside Music Data Platform ownership`);
        }
    }
}
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/music_intelligence/core"))) {
    ...
    for (const forbiddenImport of [
        ...
        if (text.includes(forbiddenImport)) {
            musicIntelligenceCoreStageImportFailures.push(`${relativeFile} imports forbidden Stage Interface contract surface '${forbiddenImport}'`);
        }
    }
}
assert.deepEqual(musicIntelligenceCoreStageImportFailures, [], "Music Intelligence core must not import Stage Interface contracts or public description helpers; stage_adapter owns that boundary");
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/music_experience"))) {
    ...
    for (const forbiddenImport of [
        ...
        if (text.includes(`from "${forbiddenImport}`) ||
            text.includes(`from '${forbiddenImport}`)) {
            musicExperienceImportFailures.push(`${relativeFile} imports forbidden Music Experience dependency '${forbiddenImport}'`);
        }
    }
    for (const mdpImport of [
        "../music_data_platform/",
        ...
        if ((text.includes(`from "${mdpImport}`) || text.includes(`from '${mdpImport}`)) &&
            !(text.includes(`from "${mdpImport}index.js"`) || text.includes(`from '${mdpImport}index.js'`))) {
            musicExperienceImportFailures.push(`${relativeFile} imports Music Data Platform internals instead of narrow public ports`);
        }
    }
    for (const importedName of musicDataPlatformIndexImportNames(text)) {
        if (!musicExperienceAllowedMdpImports.has(importedName)) {
            musicExperienceImportFailures.push(`${relativeFile} imports disallowed Music Data Platform index symbol '${importedName}'`);
        }
    }
    if (hasMusicDataPlatformIndexNamespaceImport(text)) {
        musicExperienceImportFailures.push(`${relativeFile} imports Music Data Platform index through a namespace import`);
    }
    for (const forbiddenMdpSymbol of [
    ...
    if (relativeFile !== "src/music_experience/stage_adapter/index.ts" &&
        (text.includes('from "../../stage_core/') ||
            text.includes("from '../../stage_core/"))) {
        musicExperienceImportFailures.push(`${relativeFile} imports Stage Core outside the runtime-module contribution boundary`);
    }
}
assert.deepEqual(musicExperienceImportFailures, [], "Music Experience stage adapter must consume only Stage Interface contracts, HandleMinting context, and Music Data Platform commit/projection ports");
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/music_intelligence"))) {
    ...
    for (const forbiddenRuntimeTable of [
        ...
        if (text.includes(forbiddenRuntimeTable)) {
            musicIntelligenceRuntimeResultSetFailures.push(`${relativeFile} mentions runtime result-set/cache table '${forbiddenRuntimeTable}'`);
        }
    }
}
...
for (const file of await sourceFilesUnder(join(repositoryRoot, "src/music_data_platform"))) {
    const relativeFile = relative(repositoryRoot, file);
    const text = await readFile(file, "utf8");
    if (text.includes("assertRefSafe") &&
        !rawRefPrimitiveAllowedFiles.has(relativeFile)) {
        musicDataPlatformRawRefAssertFailures.push(`${relativeFile} imports or calls contracts assertRefSafe outside ref_validation ownership`);
    }
    if (text.includes("isRefComponentSafe") &&
        !rawRefPrimitiveAllowedFiles.has(relativeFile)) {
        musicDataPlatformRawRefAssertFailures.push(`${relativeFile} imports or calls contracts isRefComponentSafe outside allowed low-level ownership`);
    }
}
assert.deepEqual(musicDataPlatformRawRefAssertFailures, []);
const musicDataPlatformBarrelText = await readFile(join(repositoryRoot, "src/music_data_platform/index.ts"), "utf8");
...
for (const file of activeFiles) {
    ...
    for (const factoryCall of [
        ...
        if (text.includes(factoryCall) &&
            ...
            repositoryFactoryFailures.push(`${relativeFile} calls low-level repository factory '${factoryCall}' outside an owning command/read/projection boundary`);
        }
    }
}
...
for (const file of activeFiles) {
    ...
    for (const factoryCall of [
        ...
        if (text.includes(factoryCall) &&
            ...
            lowLevelWriteFactoryFailures.push(`${relativeFile} calls low-level source-of-truth write factory '${factoryCall}' outside the owning write module or top-level facade`);
        }
    }
}
...
for (const file of activeFiles) {
    const relativeFile = relative(repositoryRoot, file);
    const text = await readFile(file, "utf8");
    if (text.includes(".markProjectionInvalidated(") &&
        !projectionInvalidationCallAllowedFiles.has(relativeFile)) {
        projectionInvalidationCallFailures.push(`${relativeFile} calls markProjectionInvalidated outside the owning write boundary`);
    }
}
...
for (const file of activeFiles) {
    const relativeFile = relative(repositoryRoot, file);
    const text = await readFile(file, "utf8");
    if (text.includes(".markProjectionTargetDirty(") &&
        !projectionTargetDirtyCallAllowedFiles.has(relativeFile)) {
        projectionTargetDirtyCallFailures.push(`${relativeFile} calls markProjectionTargetDirty outside Projection Maintenance ownership`);
    }
}
...
for (const file of activeFiles) {
    ...
    for (const rebuildCall of [
        ...
        if (text.includes(rebuildCall) &&
            !projectionRebuildCallAllowedFiles.has(relativeFile)) {
            projectionRebuildCallFailures.push(`${relativeFile} calls projection rebuild command '${rebuildCall}' outside Projection Maintenance runner ownership`);
        }
    }
}
...
for (const file of activeFiles) {
    const relativeFile = relative(repositoryRoot, file);
    const text = await readFile(file, "utf8");
    if (text.includes("createProjectionMaintenanceRunner")
        && !projectionMaintenanceRunnerFactoryAllowedFiles.has(relativeFile)) {
        projectionMaintenanceRunnerFactoryFailures.push(`${relativeFile} mentions Projection Maintenance runner factory outside the allowed runtime orchestration boundary`);
    }
}
...
for (const file of activeFiles) {
    ...
    for (const writeToken of [
        ...
        if (text.includes(writeToken) &&
            !directWriteAllowedFiles.has(relativeFile)) {
            directWriteFailures.push(`${relativeFile} contains write token '${writeToken}' outside an owning write boundary`);
        }
    }
}
...
for (const file of activeFiles) {
    ...
    if (text.toUpperCase().includes("DROP TABLE")) {
        schemaDestructiveResetFailures.push(`${relativeFile} contains destructive schema initialization SQL 'DROP TABLE'`);
    }
}
assert.deepEqual(schemaDestructiveResetFailures, [], "schema initialization must not hide destructive table resets");
...
const kernelLeakText = await readFile(join(repositoryRoot, "src/contracts/kernel.ts"), "utf8");
const kernelExportPattern = /\bexport\s+(?:type|interface|function|const|class|enum)\s+([A-Za-z_$][A-Za-z0-9_$]*)/gu;
const kernelExports = new Set([...kernelLeakText.matchAll(kernelExportPattern)]
    .map((match) => match[1])
...
for (const exportedName of kernelExports) {
    if (!kernelAllowedExports.has(exportedName)) {
        kernelLeakFailures.push(`src/contracts/kernel.ts exports non-kernel identifier '${exportedName}' (kernel must hold only cross-cutting primitives)`);
    }
}
for (const expectedKernelExport of kernelAllowedExports) {
    if (!kernelExports.has(expectedKernelExport)) {
        kernelLeakFailures.push(`src/contracts/kernel.ts is missing expected kernel export '${expectedKernelExport}'`);
    }
}
assert.deepEqual(kernelLeakFailures, [], "src/contracts/kernel.ts must export only the cross-cutting kernel primitives: Result, StageError, StageWarning, FormalArea, Ref, isRefComponentSafe, assertRefSafe, refKey, parseRefKey");
...
for (const sourceFile of refOriginSourceFiles) {
    ...
    for (const refPrimitive of refPrimitives) {
        const refImportPattern = new RegExp(`\\bimport(?:\\s+type)?\\s*\\{[^}]*\\b${refPrimitive}\\b[^}]*\\}\\s*from\\s*["']([^"']+)["']`, "gu");
        for (const match of text.matchAll(refImportPattern)) {
            const specifier = match[1];
            if (typeof specifier === "string" &&
                !/contracts\/kernel\.js$/u.test(specifier)) {
                refOriginFailures.push(`${relative(repositoryRoot, sourceFile)} imports '${refPrimitive}' from '${specifier}' (ref primitives must come from contracts/kernel.js)`);
            }
        }
    }
}
assert.deepEqual(refOriginFailures, [], "ref primitives (isRefComponentSafe, assertRefSafe, refKey) must be imported only from contracts/kernel.js");
...
function musicDataPlatformIndexImportNames(text: string): string[] {
    const names: string[] = [];
    const importPattern = /import\s+(?:type\s+)?\{([^}]*)\}\s+from\s+["'](?:\.\.\/){1,3}music_data_platform\/index\.js["'];/g;
    for (const match of text.matchAll(importPattern)) {
...
function exportsName(text: string, name: string): boolean {
    ...
    return new RegExp(`export\\s+(?:type\\s+)?\\{[^}]*\\b${escapedName}\\b[^}]*\\}`, "su").test(text);
}
