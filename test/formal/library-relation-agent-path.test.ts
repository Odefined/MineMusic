...
if (initializedServerModule.ok) {
  ...
  assert.deepEqual(activeRelationKinds(materialRef), []);
...
{
  const schemaDatabase = SqliteMusicDatabase.open({ filename: ":memory:" });

  schemaDatabase.initialize({
    schemas: [
      musicDataPlatformIdentitySchema,
      musicDataPlatformOwnerRelationSchema,
      musicDataPlatformProjectionMaintenanceSchema,
...
function activeRelationKinds(material: Ref): string[] {
  return database.context().all<{ relation_kind: string }>(
    `
