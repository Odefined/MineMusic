...

{
  ...
  const first = commit.commitCandidate({ materialCandidateRef: candidateRef });
  ...
  const second = commit.commitCandidate({ materialCandidateRef: candidateRef });
  ...
  assert.equal(tableCount(context, "canonical_records"), 0);
  ...
  assert.equal(
    materialRecord?.entity.primarySourceRef === undefined
      ? undefined
      : refKey(materialRecord.entity.primarySourceRef),
    refKey(source.sourceRef),
  );
  assert.deepEqual(
    repositories.sourceMaterialBindings
      .listSourcesForMaterial({ materialRef: first.value.materialRef })
      .map((binding) => refKey(binding.sourceRef)),
    [refKey(source.sourceRef)],
  );

  const projectionKinds = createProjectionMaintenanceRecords({
    db: context,
...

{
  ...
  const result = commit.commitCandidate({ materialCandidateRef: candidateRef });
  ...
  assert.equal(tableCount(database.context(), "material_records"), 0);
...

{
  ...
  const result = commit.commitCandidate({ materialCandidateRef: missingRef });
