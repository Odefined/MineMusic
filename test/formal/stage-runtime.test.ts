...
assert.equal((await failureRuntime.snapshot().modules.find((module) => module.id === "first"))?.status, "stopped");
assert.equal((await failureRuntime.snapshot().modules.find((module) => module.id === "third"))?.status, "failed");
...
assert.equal((await throwingRuntime.snapshot().modules.find((module) => module.id === "throws"))?.status, "failed");
assert.equal((await throwingRuntime.snapshot().modules.find((module) => module.id === "before-throw"))?.status, "stopped");
...
assert.equal((await stopThrowingRuntime.snapshot().modules.find((module) => module.id === "stop-throws"))?.status, "failed");
...
function testModule(id: string, events: string[], options: {
...
function contributionFor(id: string, options: {
    instrumentId?: string;
    toolName?: string;
