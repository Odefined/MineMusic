...
async function registerSourceProvider(registry: CapabilityRegistry, registration: SourceProviderRegistration): Promise<Result<void>> {
    return await registry.register(sourceProviderSlot, {
        pluginId: registration.pluginId,
        key: registration.providerId,
        value: registration.provider,
    });
}
async function getSourceProvider(registry: CapabilityRegistry, providerId: string): Promise<SourceProviderRegistration | undefined> {
    const registration = await registry.get(sourceProviderSlot, providerId);
    return registration === undefined
        ? undefined
        : { pluginId: registration.pluginId, providerId: registration.key, provider: registration.value };
}
async function registerPlatformLibraryProvider(registry: CapabilityRegistry, registration: PlatformLibraryProviderRegistration): Promise<Result<void>> {
    return await registry.register(platformLibraryProviderSlot, {
        pluginId: registration.pluginId,
        key: registration.providerId,
        value: registration.provider,
    });
...
export type _capabilitySlotShape = Expect<Equal<keyof CapabilitySlot<SourceProvider>, "id" | "cardinality" | "writePolicy" | "validateRegistration">>;
...
export type _activationContextShape = Expect<Equal<keyof PluginActivationContext, "pluginId" | "register">>;
...
assertErrorCode(validatePluginManifest({
...
assert.throws(() => validatePluginManifest(null as unknown as Parameters<typeof validatePluginManifest>[0]), TypeError);
assert.throws(() => validatePluginManifest({
    manifest: plugin("missing-known-capabilities").manifest,
} as unknown as Parameters<typeof validatePluginManifest>[0]), TypeError);
assert.throws(() => validatePluginManifest({
...
} as unknown as Parameters<typeof validatePluginManifest>[0]), /known capability lookup failed/u);
...
assert.equal((await registerSourceProvider(registry, {
    pluginId: "internal.fixture-source",
    providerId: "fixture-source",
    provider: fixtureProvider,
})).ok, true);
assert.equal((await getSourceProvider(registry, "fixture-source"))?.provider, fixtureProvider);
assert.deepEqual(await (await registry.list(sourceProviderSlot)).map((registration) => registration.key), [
    "fixture-source",
]);
const copiedRegistrations = await registry.list(sourceProviderSlot) as unknown as unknown[];
copiedRegistrations.length = 0;
assert.deepEqual(await (await registry.list(sourceProviderSlot)).map((registration) => registration.key), [
    "fixture-source",
]);
assertErrorCode(await registerSourceProvider(registry, {
    pluginId: "internal.fixture-source",
    providerId: "fixture-source",
    provider: fixtureProvider,
}), "extension.duplicate_capability_registration");
assertErrorCode(validateSourceProviderRegistration({
...
assertErrorCode(validateSourceProviderRegistration(null as unknown as SourceProviderRegistration), "extension.invalid_source_provider_registration");
assertErrorCode(validateSourceProviderRegistration({
    pluginId: "internal.bad-source",
...
assertErrorCode(await unknownRegistry.register(sourceProviderSlot, {
...
assertErrorCode(await coreOnlyRegistry.register(coreOnlySlot, {
...
assert.deepEqual((await orderedRuntime.listSourceProviders()).map((registration) => registration.providerId), [
    "first-a",
    "first-b",
    "second-a",
]);
assert.equal((await orderedRuntime.getSourceProvider("second-a"))?.provider.descriptor.providerId, "second-a");
...
assertErrorCode(await orderedRuntime.searchSourceProvider({
...
assert.equal((await lateRegistrationRuntime.listSourceProviders()).length, 1);
assertErrorCode(await capturedContext?.register(sourceProviderSlot, {
    key: "too-late",
    value: registrationFor("too-late", "internal.late-registration").provider,
}) ??
    fail("missing_context", "missing context"), "extension.activation_context_closed");
assert.equal((await lateRegistrationRuntime.listSourceProviders()).length, 1);
await assertExtensionRuntimeError(createExtensionRuntime({
    plugins: [
        {
            manifest: {
...
await assertExtensionRuntimeError(symbolManifestRuntime, "extension.invalid_plugin_id");
...
await assertExtensionRuntimeError(failedRuntime, "extension.plugin_activation_failed");
assertErrorCode(await failedRuntime.searchSourceProvider({
...
await assertExtensionRuntimeError(failedAfterRegistrationRuntime, "extension.plugin_activation_failed");
assert.equal((await failedAfterRegistrationRuntime.listSourceProviders()).length, 0);
...
assertErrorCode(await unsupportedSearchRuntime.searchSourceProvider({
    providerId: "no-search",
    query: { text: "coding" },
}), "extension.source_provider_search_unsupported");
assertErrorCode(await createExtensionRuntime({
...
assertErrorCode(await validationRuntime.searchSourceProvider({
...
for (const malformedInput of [
    ...
    assertErrorCode(await validationRuntime.searchSourceProvider(malformedInput as unknown as Parameters<typeof validationRuntime.searchSourceProvider>[0]), "extension.invalid_source_provider_search_input");
}
for (const [providerId, candidate] of [
    ...
    assertErrorCode(await runtime.searchSourceProvider({
        providerId,
...
assertErrorCode(await mutatingQueryRuntime.searchSourceProvider({
    providerId: "mutates-query",
...
const nonArrayCandidateRuntime = createExtensionRuntime({
    plugins: [
        plugin("non-array-candidates", {
            registrations: [
                registrationFor("non-array-candidates", "internal.non-array-candidates", {} as unknown as readonly ProviderMaterialCandidate[]),
            ],
        }),
    ],
});
assert.equal((await nonArrayCandidateRuntime.initialize()).ok, true);
assertErrorCode(await nonArrayCandidateRuntime.searchSourceProvider({
...
assertErrorCode(await providerFailureRuntime.searchSourceProvider({
...
assertErrorCode(await retryableProviderFailureRuntime.searchSourceProvider({
    providerId: "retryable-provider-fails",
    query: { text: "coding" },
}), "extension.source_provider_search_failed", true);
...
assertErrorCode(await providerThrowsRuntime.searchSourceProvider({
    providerId: "provider-throws",
    query: { text: "coding" },
}), "extension.source_provider_search_failed");
...
assert.equal((await registerPlatformLibraryProvider(platformRegistry, {
    pluginId: "internal.fixture-platform",
    providerId: "fixture-platform",
    provider: fixturePlatformProvider,
})).ok, true);
assert.deepEqual(await (await platformRegistry.list(platformLibraryProviderSlot)).map((registration) => registration.key), ["fixture-platform"]);
assertErrorCode(await registerPlatformLibraryProvider(platformRegistry, {
    pluginId: "internal.fixture-platform",
    providerId: "fixture-platform",
    provider: fixturePlatformProvider,
}), "extension.duplicate_capability_registration");
assertErrorCode(validatePlatformLibraryProviderRegistration({
    ...
    plugins: [
        {
            ...
            async activate(ctx) {
                const sourceRegistration = await ctx.register(sourceProviderSlot, {
                    key: "platform",
                ...
                return await ctx.register(platformLibraryProviderSlot, {
                    key: "platform",
...
assert.equal((await platformRuntime.listPlatformLibraryProviders()).length, 1);
assert.equal((await platformRuntime.getPlatformLibraryProvider("platform"))?.provider.descriptor.providerId, "platform");
...
assertErrorCode(await platformRuntime.readPlatformLibraryProvider({
    providerId: "platform",
    request: {
...
for (const malformedInput of [
    ...
    assertErrorCode(await platformRuntime.readPlatformLibraryProvider(malformedInput as unknown as Parameters<typeof platformRuntime.readPlatformLibraryProvider>[0]), "extension.invalid_platform_library_provider_read_input");
}
assertErrorCode(await platformRuntime.readPlatformLibraryProvider({
...
for (const [providerId, readResult] of [
    ...
    assert.equal((await runtime.initialize()).ok, true);
    assertErrorCode(await runtime.readPlatformLibraryProvider({
        providerId,
        request: {
            kind: "saved_source_track",
...
assertErrorCode(await platformFailureRuntime.readPlatformLibraryProvider({
    providerId: "platform-fails",
    request: { kind: "saved_source_track" },
}), "extension.platform_library_provider_read_failed", true);
const platformNotReadyRuntime = createExtensionRuntime({
    plugins: [platformOnlyPlugin("platform-not-ready")],
});
assertErrorCode(await platformNotReadyRuntime.readPlatformLibraryProvider({
    ...
    return {
        ...
        async activate(ctx) {
            ...
            for (const registration of registrations) {
                await ctx.register(sourceProviderSlot, {
                    key: registration.providerId,
                    value: registration.provider,
                });
            }
...
function registrationFor(providerId: string, pluginId = `internal.${providerId}`, candidates: readonly ProviderMaterialCandidate[] = [candidateFor(providerId, "track-1", "Coding Track")], options: {
...
function provider(providerId: string, options: {
...
function platformOnlyPlugin(providerId: string, readResult: PlatformLibraryProviderReadResult | undefined = {
    providerId,
    kind: "saved_source_track",
    candidates: [platformCandidateFor(providerId, "track-1", "Coding Track")],
}, failedResult?: Result<PlatformLibraryProviderReadResult>): MineMusicPlugin {
    return {
        manifest: {
...
function platformRegistrationFor(providerId: string, pluginId = `internal.${providerId}`, options: {
...
function platformProvider(providerId: string, options: {
...
function platformCandidateFor(providerId: string, providerEntityId: string, title: string, providerAccountId?: string): PlatformLibraryCandidate {
    return {
...
function candidateFor(providerId: string, providerEntityId: string, title: string): ProviderMaterialCandidate {
    return {
        sourceEntity: {
            kind: "track",
            sourceRef: {
...
async function assertExtensionRuntimeError(runtime: ReturnType<typeof createExtensionRuntime>, code: string): Promise<void> {
