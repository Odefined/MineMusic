...
import {
  createDownloadCommands,
  type DownloadCommands,
  type DownloadJobStatus,
  type DownloadSourceProvider,
  type MediaFileWriter,
} from "../../src/music_data_platform/download_commands.js";
import { musicDataPlatformDownloadSchema } from "../../src/music_data_platform/download_schema.js";
import { SqliteMusicDatabase, type MusicDatabase } from "../../src/storage/index.js";
...
type MemoryFiles = { writer: MediaFileWriter; files: Map<string, Uint8Array> };
...
function createFakeDownloadSourceProvider(
  resolve: (input: { providerId: string; sourceRef: Ref; preferredBitrate?: number }) => Result<DownloadSource>,
): DownloadSourceProvider & { calls: { providerId: string; preferredBitrate?: number }[] } {
  const calls: { providerId: string; preferredBitrate?: number }[] = [];
  return {
    calls,
...
async function waitForTerminal(commands: DownloadCommands, jobId: string): Promise<DownloadJobStatus> {
  for (let i = 0; i < 100; i += 1) {
    await tick();
    const status = assertOk(commands.status(jobId));
...
function openDatabase(): MusicDatabase {
  const database = SqliteMusicDatabase.open({ filename: ":memory:" });
  database.initialize({ schemas: [musicDataPlatformDownloadSchema] });
  return database;
}
...
// --- happy path: start a track download, background fetch writes the file ---
{
  ...
  const audioMd5 = createHash("md5").update(audio).digest("hex");
  ...
  assert.deepEqual(files.get("/out/song.flac"), audio);
...
// --- a non-track sourceRef is rejected before any provider call or job ---
{
  ...
  assertError(
    await commands.start({ sourceRef: albumRef, outputDir: "/out", filename: "a.flac" }),
    "music_data.download_no_audio_stream",
  );
...
// --- a download_source failure is propagated, no orphan job ---
{
  ...
  assertError(commands.status("job-1"), "music_data.download_job_not_found");
...
// --- overwrite=error rejects when the output file already exists ---
{
  ...
  assertError(
    await commands.start({ sourceRef: trackRef, outputDir: "/out", filename: "song.flac" }),
    "music_data.download_output_exists",
  );
  // existing file untouched
  assert.deepEqual(files.get("/out/song.flac"), existing);
...
// --- overwrite=overwrite replaces the existing file ---
{
  ...
  assert.deepEqual(files.get("/out/song.flac"), fresh);
