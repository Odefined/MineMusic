...
const db = SqliteMusicDatabase.open({ filename: ":memory:" });
db.initialize({ schemas: [musicDataPlatformDownloadSchema] });
...
let status = commands.status(start.value);
if (!status.ok) {
...
for (let i = 0; i < 400 && status.value.state === "running"; i += 1) {
  ...
  status = commands.status(start.value);
  if (!status.ok) {
    console.error("status failed:", status.error.message);
    process.exit(1);
  }
