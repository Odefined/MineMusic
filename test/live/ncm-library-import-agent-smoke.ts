...
import type {
  createServerHost as createServerHostFn,
} from "../../src/server/index.js";
import type {
  createStageToolContext as createStageToolContextFn,
...
if (!liveEnabled) {
  ...
  if (!started.ok) {
    ...
    const ncmSource = listed?.sources.find((source) =>
...
async function dispatch<T>(
  host: SmokeServerHost,
  ctx: SmokeStageToolContext,
  toolName: string,
  payload: unknown,
): Promise<T | undefined> {
...
async function verifyScopeListing(
  host: SmokeServerHost,
  ctx: SmokeStageToolContext,
  sourceLibraryScope: Extract<MusicScope, { kind: "source_library" }>,
): Promise<void> {
  const listed = await dispatch<MusicListScopesOutput>(host, ctx, "music.discovery.list_scopes", {
    kind: "source_library",
  });
  const found = listed?.scopes.some((scope) =>
    scope.kind === "source_library" && scope.id === sourceLibraryScope.id
  ) ?? false;
