...
async function detailInspectEnrichesMissingReleaseAppearancesFromKnowledge(): Promise<void> {
  const repository = createInMemoryCanonicalRecordRepository();
  const store = createCanonicalStore({
    repository,
    idFactory: () => "detail-enrich-subject",
    clock: () => "2026-05-27T00:00:00.000Z",
  });
  const subject = await assertOk(
    store.createProvisional({
      kind: "recording",
      label: "Detail Enrich Track",
      evidence: [sourceRef],
    }),
  );
  const mbRef: Ref = {
            ...
            items: [
              {
                ...
                providerId: "musicbrainz",
                source: { ref: mbRef },
                rootNodeId: "recording:mb-detail-enrich",
                nodes: [
                  {
                    id: "recording:mb-detail-enrich",
                    type: "recording",
                    ref: mbRef,
                    label: "Detail Enrich Track",
                    properties: { title: "Detail Enrich Track" },
                  },
                  {
                    id: "release:release-detail-enrich",
                    type: "release",
                    ref: releaseRef,
                    label: "Detail Enrich Release",
                    properties: {
                      title: "Detail Enrich Release",
                      date: "2014-02-03",
                    },
                  },
                ],
                relations: [
                  {
                    type: "release_appearance",
                    endpoints: [
                      { nodeId: "recording:mb-detail-enrich", role: "recording" },
                      { nodeId: "release:release-detail-enrich", role: "release" },
                    ],
                  },
                ],
              },
  ...
  assert(queries.length === 2, "detail enrichment should query Knowledge after the summary inspection");
  assert(
    (queries[1] as { canonicalRef?: Ref; expand?: string[] }).canonicalRef?.id === mbRef.id &&
      (queries[1] as { expand?: string[] }).expand?.join(",") === "releases",
    "detail release appearance enrichment should query the selected recording with releases expansion",
  );
  assert(detail.inspectionId === summary.inspectionId, "detail enrichment should keep the current inspection id");
  assert(detail.expiresAt === summary.expiresAt, "detail enrichment should not refresh snapshot expiry");
  assert(idCalls === 1, "detail enrichment should not create a new inspection snapshot");
  assert(
    detail.detail?.releaseAppearances?.[0]?.title === "Detail Enrich Release" &&
      detail.detail.releaseAppearances[0]?.date === "2014-02-03" &&
      detail.detail.releaseAppearances[0]?.refToken.kind === "release",
    "detail enrichment should expose compact release appearances from the selected recording lookup",
  );
}

async function detailInspectEnrichesMissingTrackPositionsFromReleaseLookup(): Promise<void> {
  const repository = createInMemoryCanonicalRecordRepository();
  const store = createCanonicalStore({
    repository,
    idFactory: () => "track-enrich-subject",
    clock: () => "2026-05-27T00:00:00.000Z",
  });
  const subject = await assertOk(
    store.createProvisional({
      kind: "recording",
      label: "Track Enrich Song",
      evidence: [sourceRef],
    }),
  );
  const mbRef: Ref = {
  ...
  const knowledge: MusicKnowledgePort = {
    query: async ({ query }) => {
      ...
      if ("canonicalRef" in query && query.canonicalRef.kind === "release") {
        return {
          ok: true,
          value: {
            items: [
              {
                ...
                rootNodeId: "release:release-track-enrich",
                nodes: [
                  {
                    id: "release:release-track-enrich",
                    type: "release",
                    ref: releaseRef,
                    label: "Track Enrich Release",
                    properties: {
                      title: "Track Enrich Release",
                      date: "2015-06-07",
                    },
                  },
                  {
                    id: "medium:release-track-enrich:1",
                    type: "medium",
                    properties: {
                      position: 1,
                      trackCount: 11,
                  ...
                  {
                    id: "track:track-enrich-song",
                    type: "track",
                    label: "Track Enrich Song",
                    properties: {
                      position: 4,
                      title: "Track Enrich Song",
                      lengthMs: 234000,
                    },
                  },
                  {
                    id: "recording:mb-track-enrich",
                    type: "recording",
                    ref: mbRef,
                    label: "Track Enrich Song",
                    properties: { title: "Track Enrich Song" },
                  },
                  {
                    id: "recording:other-track",
                    type: "recording",
                    ref: {
                      namespace: "musicbrainz",
                      kind: "recording",
                ...
                relations: [
                  {
                    type: "has_medium",
                    endpoints: [
                      { nodeId: "release:release-track-enrich", role: "release" },
                      { nodeId: "medium:release-track-enrich:1", role: "medium" },
                    ],
                  },
                  {
                    type: "has_track",
                    endpoints: [
                      { nodeId: "medium:release-track-enrich:1", role: "medium" },
                      { nodeId: "track:track-enrich-song", role: "track" },
                    ],
                  },
                  {
                    type: "represents_recording",
                    endpoints: [
                      { nodeId: "track:track-enrich-song", role: "track" },
                      { nodeId: "recording:mb-track-enrich", role: "recording" },
                    ],
                  },
                ],
              },
          ...
          items: [
            {
              ...
              providerId: "musicbrainz",
              source: { ref: mbRef },
              rootNodeId: "recording:mb-track-enrich",
              nodes: [
                {
                  id: "recording:mb-track-enrich",
                  type: "recording",
  ...
  const maintenance = createCanonicalMaintenance({
    ...
    sessionContext: createSessionContextFor(reviewSession),
    knowledge,
    idFactory: () => "track-enrich-inspection",
    clock: () => "2026-05-27T00:00:00.000Z",
  });
  const summary = await assertOk(
    maintenance.reviewInspect({
  ...
  assert(releaseToken !== undefined, "release appearance detail should provide a release token");
  ...
  assert(queries.length === 2, "track position detail should query Knowledge when tracklist facts are missing");
  assert(
    (queries[1] as { canonicalRef?: Ref; expand?: string[] }).canonicalRef?.id === releaseRef.id &&
      (queries[1] as { expand?: string[] }).expand?.join(",") === "tracklist",
    "track position enrichment should query the selected release with tracklist expansion",
  );
  assert(
    trackPositions.detail?.releaseTrackPositions?.length === 1 &&
      position?.disc === "1" &&
      position.track === 4 &&
      position.trackCount === 11 &&
      position.trackTitle === "Track Enrich Song" &&
      position.trackLengthMs === 234000,
    "track position enrichment should return only the selected recording position on the requested release",
  );
}
