...
| Material Query / Related | domain material retrieval through narrow query/projection store dependencies, direct Source Library / Collection material projection for owned assets, related candidate generation, selector delegation, materialId result handles | raw source/canonical graph exposure, provider internals, canonical writes, broad Material Store mutation authority, tag/style-hint interpretation without real semantic data, final recommendation selection, compact output projection |
