...
function createNcmSourceProvider(config) {
    return {
        descriptor: {
            providerId: ncmProviderId,
        ...
        async search({ query }) {
            ...
            if (targets.length > 1 && offset > 0) {
                return failExtension("extension.ncm_multi_kind_offset_unsupported", "NCM source search does not support offset for multi-kind search.");
            }
...
function createNcmPlatformLibraryProvider(config) {
    return {
        descriptor: {
            providerId: ncmProviderId,
            label: "NetEase Cloud Music",
...
async function readNcmSavedTracks(config, input) {
...
async function readNcmSavedAlbums(config, input) {
    const offset = cursorOffset(input.cursor);
    if (!offset.ok) {
        return offset;
    }
    ...
    const albums = ncmLibraryArray(payload.value, ["data", "albums"]);
    if (!albums.ok) {
        return albums;
    }
...
async function readNcmFollowedArtists(config, input) {
    const offset = cursorOffset(input.cursor);
    if (!offset.ok) {
        return offset;
    }
    ...
    const artists = ncmLibraryArray(payload.value, ["data", "artists"]);
    if (!artists.ok) {
        return artists;
    }
...
async function requestNcmSearch(config, input) {
    if (!isRecord(config)) {
        return failExtension("extension.ncm_invalid_config", "NCM plugin config must be an object.");
    }
    ...
    if (typeof fetchJson !== "function") {
        return failExtension("extension.ncm_invalid_config", "NCM plugin fetch config must be a function.");
    }
    ...
    try {
        ...
        return failExtension("extension.ncm_provider_unavailable", `NCM provider is unavailable at ${url.origin}.`, undefined, true);
    }
    if (!response.ok) {
        return failExtension("extension.ncm_provider_unavailable", `NCM provider returned HTTP ${response.status}.`, undefined, true);
    }
    try {
        return ok(await response.json());
    }
    catch {
        return failExtension("extension.ncm_malformed_response", "NCM provider returned malformed JSON.");
    }
}
async function requestNcmPath(config, path, params) {
    if (!isRecord(config)) {
        return failExtension("extension.ncm_invalid_config", "NCM plugin config must be an object.");
    }
    const fetchJson = config.fetch ?? fetch;
...
async function resolveNcmProviderAccountId(config, providerAccountId) {
    const explicit = providerAccountId;
    if (explicit !== undefined && !isSafeId(explicit)) {
        return failExtension("extension.ncm_invalid_provider_account_id", "NCM providerAccountId must be a non-empty safe id when present.");
    }
    ...
    if (accountId === undefined) {
        return failExtension("extension.ncm_account_unresolved", "NCM provider could not resolve the current logged-in account.", undefined, true);
    }
    if (explicit !== undefined && explicit !== accountId) {
        return failExtension("extension.ncm_account_mismatch", "NCM providerAccountId does not match the current logged-in account.", undefined, true);
    }
    return ok(accountId);
}
async function resolveNcmLikedPlaylistId(config, providerAccountId) {
    ...
    const playlists = ncmLibraryArray(payload.value, ["playlist"]);
    if (!playlists.ok) {
        return playlists;
    }
    ...
    if (id === undefined) {
        return failExtension("extension.ncm_liked_playlist_unresolved", "NCM provider could not resolve the liked music playlist.", undefined, true);
    }
    return ok(id);
}
async function readNcmSongs(config, ids) {
    ...
    return ncmLibraryArray(payload.value, ["songs"]);
}
function ncmAccountId(payload) {
    if (!isRecord(payload)) {
        return undefined;
    }
...
function ncmLikedTrackIds(payload) {
    const playlist = isRecord(payload) && isRecord(payload.playlist)
        ? payload.playlist
        : undefined;
    const trackIds = playlist?.trackIds;
    if (!Array.isArray(trackIds)) {
        return failExtension("extension.ncm_malformed_response", "NCM provider response did not include playlist.trackIds.");
    }
    const ids = [];
    for (const item of trackIds) {
        if (!isRecord(item)) {
            return failExtension("extension.ncm_malformed_response", "NCM provider playlist.trackIds included a non-object item.");
        }
        const id = toUsableProviderId(item.id);
        const providerAddedAt = dateTimeFromNcmTime(item.at);
        if (id === undefined) {
            return failExtension("extension.ncm_malformed_response", "NCM provider playlist.trackIds included an item without a usable id.");
        }
        ids.push({
            id,
...
function toSavedTrackCandidates(selected, songs, providerAccountId) {
    const songsById = new Map(songs.map((song) => [toUsableProviderId(song.id), song]));
    const candidates = [];
    for (const item of selected) {
        const song = songsById.get(item.id);
        if (song === undefined) {
            return failExtension("extension.ncm_song_detail_missing", `NCM provider response did not include song detail for track '${item.id}'.`);
        }
        const candidate = toTrackCandidate(song);
        if (candidate === undefined) {
            return failExtension("extension.ncm_malformed_response", `NCM provider song detail for track '${item.id}' did not include usable source facts.`);
        }
        candidates.push(platformCandidate("saved_source_track", candidate, providerAccountId, item.providerAddedAt));
    }
    return ok(candidates);
}
function toSavedAlbumCandidates(albums, providerAccountId) {
    const candidates = [];
    for (const album of albums) {
        const candidate = toAlbumCandidate(album);
        if (candidate === undefined) {
            return failExtension("extension.ncm_malformed_response", "NCM provider saved album item did not include usable source facts.");
        }
        candidates.push(platformCandidate("saved_source_album", candidate, providerAccountId, dateTimeFromNcmTime(album.subTime)));
    }
    return ok(candidates);
}
function toFollowedArtistCandidates(artists, providerAccountId) {
    const candidates = [];
    for (const artist of artists) {
        const candidate = toArtistCandidate(artist);
        if (candidate === undefined) {
            return failExtension("extension.ncm_malformed_response", "NCM provider followed artist item did not include usable source facts.");
        }
        candidates.push(platformCandidate("followed_source_artist", candidate, providerAccountId));
    }
    return ok(candidates);
}
function ncmLibraryArray(payload, paths) {
    if (!isRecord(payload)) {
        return failExtension("extension.ncm_malformed_response", "NCM provider response was not an object.");
    }
    for (const path of paths) {
        const value = payload[path];
        if (Array.isArray(value)) {
            if (!value.every(isRecord)) {
                return failExtension("extension.ncm_malformed_response", `NCM provider response ${path} included a non-object item.`);
            }
            return ok(value);
        }
    }
    return failExtension("extension.ncm_malformed_response", `NCM provider response did not include ${paths.join(" or ")}.`);
}
function cursorOffset(cursor) {
    ...
    if (!/^(0|[1-9]\d*)$/.test(cursor)) {
        return failExtension("extension.ncm_invalid_cursor", "NCM platform library cursor must be a non-negative integer offset.");
    }
...
function validateNonEmptyHasMorePage(payload, itemCount, itemLabel) {
    if (hasMore(payload) && itemCount === 0) {
        return failExtension("extension.ncm_malformed_response", `NCM provider returned hasMore with an empty ${itemLabel} page.`);
    }
    return ok(undefined);
}
...
function platformCandidate(libraryKind, candidate, providerAccountId, providerAddedAt) {
    return {
...
function ncmSearchUrl(baseUrl) {
    return ncmPathUrl(baseUrl, "/search");
}
function ncmPathUrl(baseUrl, path) {
    if (typeof baseUrl !== "string" || baseUrl.trim().length === 0) {
        return failExtension("extension.ncm_invalid_config", "NCM plugin baseUrl config must be a non-empty URL string.");
    }
    try {
        return ok(new URL(path, normalizedBaseUrl(baseUrl.trim())));
    }
    catch {
        return failExtension("extension.ncm_invalid_config", "NCM plugin baseUrl config must be a valid URL.");
    }
}
function mapSearchPayload(kind, payload) {
    ...
    if (!isRecord(result)) {
        return failExtension("extension.ncm_malformed_response", "NCM provider response did not include result.");
    }
    switch (kind) {
        case "track":
...
function mapPayloadArray(value, path, mapper) {
    if (!Array.isArray(value)) {
        return failExtension("extension.ncm_malformed_response", `NCM provider response did not include ${path}.`);
    }
    return ok(value.filter(isRecord).map((item) => mapper(item)).filter(isDefined));
}
function toTrackCandidate(song) {
...
function toAlbumCandidate(album) {
...
function toArtistCandidate(artist) {
...
function issueFromNcmPayload(payload) {
    if (!isRecord(payload)) {
        return failExtension("extension.ncm_malformed_response", "NCM provider response was not an object.");
    }
    ...
    return failExtension("extension.ncm_provider_response_error", message);
}
function normalizedTargets(targetKinds) {
...
function splitLimit(limit, partCount) {
    if (limit === undefined) {
...
function defaultNcmSearchLimit() {
    return 10;
}
function defaultNcmLibraryLimit() {
    return 50;
}
function artistFacts(artists) {
    const labels = [];
    const refs = [];
...
function artistRecords(primary, fallback) {
    const primaryRecords = arrayRecords(primary);
    const fallbackRecords = arrayRecords(fallback);
...
function albumArtistRecords(album) {
    return artistRecords(album.artists, isRecord(album.artist) ? [album.artist] : undefined);
}
function albumRecord(primary, fallback) {
    const primaryAlbum = isRecord(primary) ? primary : undefined;
    const fallbackAlbum = isRecord(fallback) ? fallback : undefined;
...
function toAlbumSourceRef(album) {
    const id = toUsableProviderId(album.id);
...
function toTrackPosition(song, album) {
...
function trackLink(id, requiresAccount) {
    return {
        url: songUrl(id),
        label: "NetEase Cloud Music",
...
function extractVersionInfo(values) {
...
function explicitVersionPhrases(value) {
    if (value === undefined) {
        return [];
    }
    const phrases = [];
...
function versionTagsForPhrase(phrase) {
...
function releaseDateFromNcmTime(value) {
...
function dateTimeFromNcmTime(value) {
    if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) {
        return undefined;
    }
...
function labelWithArtists(title, artists) {
    return artists.length === 0 ? title : `${title} - ${artists.join(", ")}`;
}
function toUsableProviderId(value) {
