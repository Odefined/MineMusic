...
import { createExtensionRuntimeModule, createStageRuntime, isRuntimeModuleIdSafe, } from "../../src/stage_core/index.js";
...
if (statusDispatch.ok) {
    assert.deepEqual(Object.keys(statusDispatch.value.result).sort(), [
        "interface",
        "modules",
        "status",
    ]);
    const output = statusDispatch.value.result;
...
if (extensionStatusDispatch.ok) {
    const output = extensionStatusDispatch.value.result;
    assert.deepEqual(Object.keys(extensionStatusDispatch.value.result).sort(), [
        "interface",
        "modules",
        "status",
    ]);
...
function testModule(id, events, options = {}) {
    return {
        descriptor: {
            id,
            ownerArea: "stage_core",
...
function contributionFor(id, options = {}) {
