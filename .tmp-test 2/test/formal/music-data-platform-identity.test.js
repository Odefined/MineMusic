...
import { createIdentityRepositories, createIdentityWriteCommands, isMusicDataPlatformError, musicDataPlatformIdentitySchema, } from "../../src/music_data_platform/index.js";
...
database.transaction((db) => {
    ...
    assertMusicDataError(() => commands.upsertSourceRecord({
        entity: {
    ...
    assertMusicDataError(() => commands.upsertMaterialRecord({
        materialRef: {
            namespace: "material",
            kind: "album",
    ...
    assertMusicDataError(() => commands.bindMaterialToCanonical({
        materialRef: materialRef("material-1"),
    ...
    assertMusicDataError(() => commands.upsertCanonicalRecord({
        entity: {
    ...
    assertMusicDataError(() => commands.bindSourceToMaterial({
...
database.transaction((db) => {
    ...
    assert.equal(refKey(requiredBinding(repositories.sourceMaterialBindings.findMaterialForSource({
        sourceRef: sourceRef("source-1"),
...
database.transaction((db) => {
    ...
    assertMusicDataError(() => commands.mergeMaterialRecord({
...
assert.equal(rollbackDatabase.context().get("SELECT COUNT(*) AS count FROM source_records")?.count, 0);
assert.equal(rollbackDatabase.context().get("SELECT COUNT(*) AS count FROM source_material_bindings")?.count, 0);
...
function canonicalEntity(id, label) {
    return {
...
function sourceRefWithKind(kind, id) {
    return {
        namespace: "source_netease",
        kind,
        id,
...
function assertMusicDataError(operation, code) {
    assert.throws(operation, (error) => isMusicDataPlatformError(error) && error.code === code);
}
