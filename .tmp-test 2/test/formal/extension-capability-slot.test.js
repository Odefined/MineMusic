...
import { createCapabilityRegistry, createExtensionRuntime, defineCapabilitySlot, getSourceProvider, isPluginIdSafe, platformLibraryProviderSlot, registerSourceProvider, registerPlatformLibraryProvider, sourceProviderSlot, validatePluginManifest, validatePlatformLibraryProviderRegistration, validateSourceProviderRegistration, } from "../../src/extension/index.js";
...
for (const input of [
    ...
    assertErrorCode(validatePluginManifest(input), "extension.invalid_plugin_manifest");
}
...
assert.equal(registerSourceProvider(registry, {
...
const copiedRegistrations = registry.list(sourceProviderSlot);
copiedRegistrations.length = 0;
assert.deepEqual(registry.list(sourceProviderSlot).map((registration) => registration.key), [
    "fixture-source",
]);
assertErrorCode(registerSourceProvider(registry, {
    pluginId: "internal.fixture-source",
    providerId: "fixture-source",
    provider: fixtureProvider,
...
assertErrorCode(validateSourceProviderRegistration(null), "extension.invalid_source_provider_registration");
assertErrorCode(validateSourceProviderRegistration({
    pluginId: "internal.bad-source",
    providerId: Symbol("bad-source"),
    provider: provider("bad-source"),
}), "extension.unsafe_provider_id");
...
assertErrorCode(unknownRegistry.register(sourceProviderSlot, {
    pluginId: "internal.unknown-slot",
    key: "fixture-source",
    value: fixtureProvider,
}), "extension.unknown_capability");
const coreOnlySlot = defineCapabilitySlot({
    id: "core-only-test",
    cardinality: "single",
...
assertErrorCode(coreOnlyRegistry.register(coreOnlySlot, {
...
assertErrorCode(capturedContext?.registerSourceProvider(registrationFor("too-late", "internal.late-registration")) ??
...
for (const malformedInput of [
    ...
    assertErrorCode(await validationRuntime.searchSourceProvider(malformedInput), "extension.invalid_source_provider_search_input");
}
...
const nonArrayCandidateRuntime = createExtensionRuntime({
    plugins: [
        plugin("non-array-candidates", {
            registrations: [
                registrationFor("non-array-candidates", "internal.non-array-candidates", {}),
            ],
        }),
    ],
});
...
assert.equal(registerPlatformLibraryProvider(platformRegistry, {
    pluginId: "internal.fixture-platform",
    providerId: "fixture-platform",
    provider: fixturePlatformProvider,
}).ok, true);
assert.deepEqual(platformRegistry.list(platformLibraryProviderSlot).map((registration) => registration.key), ["fixture-platform"]);
assertErrorCode(registerPlatformLibraryProvider(platformRegistry, {
    pluginId: "internal.fixture-platform",
    providerId: "fixture-platform",
    provider: fixturePlatformProvider,
}), "extension.duplicate_capability_registration");
...
for (const malformedInput of [
    ...
    assertErrorCode(await platformRuntime.readPlatformLibraryProvider(malformedInput), "extension.invalid_platform_library_provider_read_input");
}
assertErrorCode(await platformRuntime.readPlatformLibraryProvider({
...
for (const [providerId, readResult] of [
    ...
    const runtime = createExtensionRuntime({
        plugins: [platformOnlyPlugin(providerId, readResult)],
    });
    assert.equal((await runtime.initialize()).ok, true);
...
function registrationFor(providerId, pluginId = `internal.${providerId}`, candidates = [candidateFor(providerId, "track-1", "Coding Track")], options = {}) {
    return {
        pluginId,
        providerId,
        provider: provider(providerId, {
...
function badProviderFor(providerId) {
    switch (providerId) {
...
function platformOnlyPlugin(providerId, readResult = {
    providerId,
    kind: "saved_source_track",
...
function platformRegistrationFor(providerId, pluginId = `internal.${providerId}`, options = {}) {
    return {
        pluginId,
        providerId,
...
function platformProvider(providerId, options = {}) {
    return {
        descriptor: {
            providerId,
            label: providerId,
...
function badPlatformProviderFor(providerId) {
    switch (providerId) {
        case "missing-descriptor":
            return {};
        case "missing-label":
        ...
        case "unknown-library-kind":
            return platformProvider(providerId, {
                libraryKinds: ["saved_source_track", "saved_source_release"],
            });
...
function platformCandidateFor(providerId, providerEntityId, title, providerAccountId) {
    return {
...
function candidateFor(providerId, providerEntityId, title) {
    return {
        sourceEntity: {
            kind: "track",
            sourceRef: {
...
async function assertExtensionRuntimeError(runtime, code) {
...
function assertErrorCode(result, code, retryable = false) {
