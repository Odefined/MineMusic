...
import { isMusicDatabaseError, SqliteMusicDatabase, } from "../../src/storage/index.js";
if (false) {
    const typeOnlyDatabase = undefined;
    ...
    const typeOnlySchema = {
        id: "sync-schema",
        apply() { },
    };
    void typeOnlySchema;
    const typeOnlyAsyncSchema = {
        id: "async-schema",
...
assertDatabaseError(() => SqliteMusicDatabase.open({ filename: "" }), "storage.invalid_database_filename");
assertDatabaseError(() => SqliteMusicDatabase.open({ filename: "   " }), "storage.invalid_database_filename");
...
assertDatabaseError(() => preInitDatabase.transaction(() => undefined), "storage.database_not_initialized");
...
assertDatabaseError(() => orderedDatabase.initialize(), "storage.database_already_initialized");
const pragmaContext = orderedDatabase.context();
assert.equal(pragmaContext.get("PRAGMA foreign_keys")?.foreign_keys, 1);
assert.equal(pragmaContext.get("PRAGMA synchronous")?.synchronous, 1);
assert.equal(typeof pragmaContext.get("PRAGMA journal_mode")?.journal_mode, "string");
...
assert.deepEqual(pragmaContext.all("SELECT label FROM first_schema WHERE label > ? ORDER BY label", ["a"])
    .map((row) => row.label), ["alpha", "beta"]);
assert.equal(pragmaContext.get("SELECT label FROM first_schema WHERE label = ?", ["alpha"])?.label, "alpha");
assert.equal(pragmaContext.get("SELECT label FROM first_schema WHERE label = ?", ["missing"]), undefined);
const blobPayload = new Uint8Array([1, 2, 3]);
pragmaContext.run("INSERT INTO blob_schema (payload) VALUES (?)", [blobPayload]);
const storedBlob = pragmaContext.get("SELECT payload FROM blob_schema");
...
assert.equal(pragmaContext.get("SELECT COUNT(*) AS count FROM first_schema WHERE label = ?", ["committed"])
    ?.count, 1);
...
assert.equal(pragmaContext.get("SELECT COUNT(*) AS count FROM first_schema WHERE label = ?", ["rolled-back"])
    ?.count, 0);
orderedDatabase.transaction((context) => {
    context.run("INSERT INTO first_schema (label) VALUES (?)", ["after-rollback"]);
});
assert.equal(pragmaContext.get("SELECT COUNT(*) AS count FROM first_schema WHERE label = ?", ["after-rollback"])
    ?.count, 1);
let autoRollbackError;
assert.throws(() => {
    orderedDatabase.transaction((context) => {
...
assert.equal(pragmaContext.get("SELECT COUNT(*) AS count FROM unique_schema WHERE label = ?", ["after-auto-rollback"])?.count, 1);
...
let lateAsyncTransactionWrite;
const promiseReturningTransaction = ((context) => {
...
assertDatabaseError(() => asyncTransactionDatabase.transaction(promiseReturningTransaction), "storage.async_callback_not_supported");
...
await assert.rejects(observedLateAsyncTransactionWrite, (error) => isMusicDatabaseError(error) && error.code === "storage.transaction_context_inactive");
const unhandledAsyncTransactionRejection = await observeUnhandledRejection(async () => {
    const unobservedPromiseReturningTransaction = ((context) => {
    ...
    assertDatabaseError(() => asyncTransactionDatabase.transaction(unobservedPromiseReturningTransaction), "storage.async_callback_not_supported");
});
assert.equal(unhandledAsyncTransactionRejection, undefined);
assert.equal(asyncTransactionDatabase.context().get("SELECT COUNT(*) AS count FROM async_transaction_fixture")?.count, 0);
asyncTransactionDatabase.close();
orderedDatabase.close();
orderedDatabase.close();
assertDatabaseError(() => orderedDatabase.context(), "storage.database_closed");
assertDatabaseError(() => orderedDatabase.transaction(() => undefined), "storage.database_closed");
...
assertDatabaseError(() => failingDatabase.transaction(() => undefined), "storage.database_initialization_failed");
...
assertDatabaseError(() => closeDuringInitializationDatabase.context(), "storage.database_initialization_failed");
closeDuringInitializationDatabase.close();
closeDuringInitializationDatabase.close();
...
assert.equal(firstIdempotentDatabase.context().get("SELECT COUNT(*) AS count FROM idempotent_schema WHERE label = ?", ["seed"])?.count, 1);
...
assert.equal(secondIdempotentDatabase.context().get("SELECT COUNT(*) AS count FROM idempotent_schema WHERE label = ?", ["seed"])?.count, 1);
...
assert.equal(blockedDatabase.context().get("SELECT COUNT(*) AS count FROM lock_fixture WHERE label = ?", ["after-begin-failure"])?.count, 1);
blockedDatabase.close();
lockingDatabase.close();
await assertRawSqliteBoundary();
function schema(id, order, apply) {
    return {
        id,
        apply(context) {
            order.push(id);
...
function assertDatabaseError(operation, code) {
    assert.throws(operation, (error) => isMusicDatabaseError(error) && error.code === code);
}
async function assertRawSqliteBoundary() {
...
async function observeUnhandledRejection(operation) {
    let observed;
