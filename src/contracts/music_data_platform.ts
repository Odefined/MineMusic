...
const prefixTokenPattern = /[\p{L}\p{N}_]+/gu;

export function tokenizePrefixText(text: string): readonly string[] {
  return text.match(prefixTokenPattern) ?? [];
}

export function hasPrefixToken(text: string): boolean {
  return tokenizePrefixText(text).length > 0;
}
