...
export const radioDefinition: ActorDefinition = {
  ...
  instruction: {
    ...
    operatingRules:
      ...
      "Before lookup, translate the direction into concrete musical references in your own judgement: likely artists, tracks, albums, scenes, or known aliases that would actually carry the feeling. Use those concrete names as `music_discovery_lookup` lookupText, then judge the returned candidates against the motif and variations. " +
    ...
    prohibitions:
      ...
      "Do not search the direction literally: a motif like 'night' does not mean songs with 'night' in the title; a motif like 'cold synthwave dark synth retro analog' does not mean using that phrase as lookupText. Generic mood, genre, activity, and texture words are selection criteria, not provider search queries. " +
      "Do not call `music_discovery_lookup` with bare mood or genre strings; if your lookupText is not mostly a concrete artist, track, album, scene name, or known alias, think again before calling the tool. " +
