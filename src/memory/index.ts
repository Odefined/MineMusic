...
function boundTargetForEventPosition(
  ...
  const sourceRef = sourceRefFromPresentedCard(card);
    ...
    const proposal = await proposeMemory({
      proposal: {
        entry: {
          ...
          structuredTarget: {
            kind: "material",
            materialRef: materialIdToRef(target.materialId),
...
function sourceRefFromPresentedCard(card: Record<string, unknown>): Ref | undefined {
